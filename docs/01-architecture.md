...
def sample_weight(sample, query_date):
    ...
    # 半衰期衰减 0.5 ** (age/半衰期); 与 docs/05 衰减谱 + 书架视图 recency_weight 一致。
    # (写成 exp(-age/H) 是 e-folding、≠ 半衰期, 别混用 —— codex PR#58。)
    surface_weight  = 0.5 ** (age_months / 6)    # 半衰期 6 个月
    essence_weight  = 0.5 ** (age_months / 60)   # 半衰期 5 年
...
- 1 年前的样本：surface 权重 ≈ 0.25，essence 权重 ≈ 0.87，audience 权重 ≈ 0.76
- 2 年前的样本：surface 权重 ≈ 0.063，essence 权重 ≈ 0.76，audience 权重 ≈ 0.57

老数据的 surface 信号大幅衰减（2 年≈0.06），但 essence 几乎全部保留（2 年≈0.76）。这就是数据库能"穿越周期"的算法机制。
