...
>
> 本文 = 愿景 + 技术选型 + 数据来源 + 看板内容设计 + 安全 + 分阶段落地 + 部署/域名。配套当前状态见 [docs/22](archive/22-handover-2026-06-05-onboarding-hardened.md)。
...
### ✦ 开篇视效 / 落地页(`/` · 品牌动态介绍 + 入口)⭐
- **诉求**:进站第一屏是一个**很厉害的动态视效**,介绍公司(BYWOOD 芭梧)和产品;像登录界面,但**不登录也可直接进入公众看板**。
- **信息源**:公司名片 PDF → 抽成 `dashboard/config/brand.ts`(品牌名/标语/增长链路 心智→决策→复利/扶摇 ROC 方法论/为什么越久越值钱/"不是堆人头是结构性复利")。**信息以此为准,设计=动态编排(不照搬名片排版)。**
- **必须动态**:用 framer-motion 编排入场(staggered 逐项淡入上移)+ 背景旋转双飞轮(呼应"数据飞轮·越用越强")+ 流动渐变深底 + BYWOOD 蓝/红配色。后续可加蓝词扫光 / 增长链路连线动画 / 计数。
- **两入口**:① **进入公众看板 →**(`/console`,免登录,主按钮)② **登录**(内部页,Phase 3 接 auth,现为占位)。
- **路由结构**:`/`=开篇视效 · `/console`=公众看板(数据 overview)· 未来 `/internal`=登录后运维页。
- **现状(Phase 0 已落)**:`app/page.tsx`(开篇视效)+ `app/console/page.tsx`(公众看板)+ `config/brand.ts`。动效是 v1,新窗口可在 `npm run dev` 下实时打磨到"很厉害"。
