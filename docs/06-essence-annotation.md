...
Essence 标注必须区分两种模式，**严格隔离**：

### Mode A · `prediction_feature` （用于模型训练特征）

- LLM 输入：**只有内容本身** + 项目元数据
- LLM **严禁看到**：`tier` / `impressions` / `reads` / `interactions` / 任何 performance 信号
...
- LLM 输入：内容 + **已知 tier 和 performance 数据**
- LLM 被要求分析"为什么这条爆了 / 为什么失败"
- 标注结果存入独立 `posthoc_analyses` 表
- ❌ **禁止用于训练**

### 为什么必须分模式

如果 LLM 标注 essence 时已经知道"这条爆了"，它会**事后合理化**——倾向于标更高的 emotional_intensity / 更精确的 audience / 更强的 human_truth。后续训练"预测爆不爆"的模型会把这些**后验偏见**当成"前置特征"学进去，部署后预测全是"高质量"——因为这是模型见过的爆款长相。这是经典 label leakage 错误。
...
参见: [DECISIONS.md](../DECISIONS.md) D-017
...
完整 prompt 见 [../prompts/essence_annotator.md](../prompts/essence_annotator.md)（**v0.3 双模式版**）。这里说明设计原则。

> ⚠️ v0.3 关键变更（D-028）：prompt 文件拆为 Mode A / Mode B 两个独立模板。Mode A 物理上不含 `{performance_context}` 占位符，从代码层面杜绝 label leakage。详见 prompt 文件。

### 输入 context（Mode A · prediction_feature）
...
**注意**: Mode A 严禁包含 tier / impressions / reads / interactions / 任何 performance 信号。这是为了防止 label leakage（D-017）。

### 输入 context（Mode B · posthoc_explanation）

仅用于复盘分析，结果存入 `posthoc_analyses` 表，**不污染训练特征**。
...
任务：基于已知结果，分析为什么这条爆了 / 失败。
```

Mode B 标注产出存到独立表，**不**回填到 notes 主表的 essence 字段。
