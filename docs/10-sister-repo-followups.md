...
> 3. aw cron 每日 04:00 UTC 跑 `tv-sync --all --write-tv`。⚠️ TV 夜跑 cron 声明 `0 2 * * *`，但 GitHub 排程实际
>    起跑 06:36–08:33 UTC（最近 20 次 schedule 运行的创建时间），所以 aw 那趟跑在 TV 之前，新笔记**隔天**才对上；
>    要同天对上，aw 挪到 ≥ 09:30 UTC 或 TV 把 cron 挪到非整点分钟（待 owner 拍板）。
...
> lineage FK self-check (R-031)`; docs/11 lineage 段改为"✅ 现状"; RISKS R-031 标已解决。
> 下面为原方案 (已照此落地, 保留作背景)。
