...
### Step 5 · Tier 的数值兜底（不用配，1 分钟看懂）

**2026-09-17 起（D-062）不再按项目配互动量阈值。** 运营口径：判爆**只看评论数**，全项目统一 ——

| 档位 | 评论数 |
|---|---|
| 大爆 | ≥ 100 |
| 爆 | ≥ 50 |
| 评估中 | 20–50（正式档位，不是「还没下结论」） |
| 趴 | < 20 |

同步脚本在**没有人工 tier**（状态列为空或映成 未知）时按 `comments_count` 只向上推 爆/大爆；
不推 趴/评估中（新帖评论数天然少）。**有人工 tier 时永远用人工的。**

接表时只需要一件事：把评论数列映到 `comments_count` ——

```yaml
field_mapping:
  实时数据.评论数: comments_count    # 新模板表(巡查机器人抓的实时总数, 含铺评)
  # 或
  评论数: comments_count             # 老表的累计评论数
```

没有评论数列的表 → 不做数值推断（互动量不是判爆依据，拿它猜就是 D-062 之前的错）。
`tier_thresholds` / `tier_threshold_override` 两个键**已废**，CI 会拒绝含这两个键的 mapping。
...
- [ ] schema_family 正确判断
- [ ] 所有「方向」取值都有映射
- [ ] 评论数列映到了 comments_count（没有就写明「本表无评论数, 不做数值推断」）
- [ ] 表结构上产不出的要害字段（曝光/阅读/方向…）写进了 `capabilities_absent` 并带理由 —— 不写的话 D-056 每次同步都红；表里有了这列就删声明（留着会以「豁免已过期」判红）
- [ ] 合规模板按 category 加载
- [ ] 字段映射无 "未识别" 状态
- [ ] 试导入 10 条样本无错误
