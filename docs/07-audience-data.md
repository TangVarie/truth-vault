...
1. **客户数据归客户所有**: 蒲公英数据是客户在小红书的账号数据，使用前确认合同条款是否允许
2. **匿名化聚合**: 跨客户做 audience pattern 分析时，用聚合数据（"保健品品类 35+ 女性平均占比"），不暴露具体客户笔记
3. **数据隔离**: 每个项目的 actual_audience_data 严格按 project_id 隔离，Supabase Row Level Security 配好

**合规问题 Q5**（[CURRENT_STATE.md](../CURRENT_STATE.md)）: 处方药客户（如力克雷）拉数据前要不要先和客户对齐？建议：先咨询合同部门，把"投放结果数据匿名化用于服务质量提升"作为标准条款加入新客户合同。
