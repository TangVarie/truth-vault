...
4. 复制时带了首尾空格 / 换行
5. `truth_vault` / `autowriter` 没加进 Supabase 的 **Exposed schemas** (Settings →
   API → Exposed schemas) → PostgREST 报 404「schema must be one of the following」,
   症状和坑 1（指错项目）**一模一样**。(这是 CURRENT_STATE 待启动清单里的一项。)

> 另: feishu 那步还需要每个 mapping yaml 配好 `sync_config.feishu_app_token` /
> `feishu_table_id`。缺了只让 `feishu_sync` 单独失败, 其余 4 步不受影响 —— 跟"5 步
> 全挂"是两回事, 别混。
