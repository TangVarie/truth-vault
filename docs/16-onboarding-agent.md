...
## 批量接表（2026-08-28 扩展 · 见 [DECISIONS D-044](../DECISIONS.md)）

本文档原本假设「一次一张表」。实际接到第 8 条战线之后，痛点变成**一次要接一批**——
而按钮点 6 次不只是麻烦，它让 6 次之间的失败散在 6 个 run 里，没人汇总。

扩展的形状（不改上面任何一条逻辑，只改**编排**与**入口**）：

| 维度 | 单张（原） | 批量（新） |
|---|---|---|
| 编排 | workflow 里几行 bash | `onboarder/batch.py`，逐表打同一个 `/onboard` |
| 表标识 | `app_token` + `table_id` | 直接贴**飞书链接**（`/base/` 与 `/wiki/` 都认） |
| 产出 | 一个分支 `onboarder/draft-<id>` | 一个分支 `onboarder/batch-<run_id>` + 汇总 |
| 失败 | 整个 run 红 | **逐表隔离**，汇总里给可粘贴的重跑清单 |

三条设计约束（完整论证在 D-044，这里只记结论）：

- **批量不做成服务端端点。** 单表已逼近请求超时，而 HTTP 请求是原子的 —— 服务端串起
