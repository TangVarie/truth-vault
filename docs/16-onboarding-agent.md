...
| **输入路径** | **飞书 API 自动拉** | `lark-oapi` 拉表的 fields + sample rows。凭证走 GitHub Secrets。 |
| 模型 | `claude-sonnet-4-6`（中转站已有） | 与 librarian 同款；难分析可临时上 Opus。 |
| 触发 | GitHub Actions `workflow_dispatch` | 填 `project_id` + 飞书 `table_id`。 |
...
`mappings/WTG_phase1.yaml`）。agent 用 `lark-oapi` 拉**列名清单 + N 行样本**
（默认 N=30，够枚举方向/状态取值 + 看文案样本）。
...
```
onboarder/
├── clients.py    Supabase(service_role) + Anthropic(中转站) + 飞书(lark-oapi) 客户端
├── vocab.py      受控词表闭集（8 content_format / 11 target_audience）+ 校验器（硬护栏）
├── corpus.py     加载 mappings/*.yaml + docs/03 家族指纹 → agent few-shot 上下文
├── core.py       agent 编排：拉表 → 起草 → dry import → 出 draft yaml + review brief
├── eval_wtg.py   WTG 金标准 eval（见 §验收）
├── cli.py        命令行入口（--dry-run 看 prompt；真跑连飞书+库+LLM）
└── README.md
.github/workflows/
└── onboard-table.yml   workflow_dispatch(project_id, feishu_table_id) → 跑 core → 开 PR
```

工具（agent 可调）：飞书拉表（lark-oapi）· 读 `mappings/` + `docs/03,05` ·
`recommend_tier_thresholds` 逻辑 · dry-run 导入（复用
`sync_feishu_notes_to_truth_vault.py` 的翻译/校验）。

## 验收 · WTG 金标准 eval

你已经有一份**人工确认过**的 `mappings/WTG_phase1.yaml`，正好当回归基准。

`eval_wtg.py` 让 agent **重跑 WTG**（同一张飞书表 / 同样的样本），然后对：

- **结构部分必须对上**：`field_mapping`（35 列全交代、13 typed + 22 raw_extra）、
  `tier_extraction` 规则、互动量分布推荐阈值、`schema_family=A` 判定、识别出
  "状态拆两列""观众分析列"等反常点。
- **`[待确认]` 覆盖必须一致**：agent 要把同样的判断项（6 方向拆解、阈值、合规、brand/
  product）标成待确认，**不能擅自拍死**。

通过判据：结构部分 diff = 0 关键差异，且 `[待确认]` 项集合 ⊇ WTG 当时标的。
对上 → 证明它对下一张新表可用。
...
- **样本代表性**：默认拉 30 行；方向取值多的表（如 NRT ~15 标签）可能要调大 N。
