...
```
onboarder/
├── clients.py    飞书 Bitable REST（镜像 scripts 的 FeishuClient）+ Supabase 客户端
├── vocab.py      受控词表闭集（8 content_format / 11 target_audience）+ 校验器（硬护栏）
├── corpus.py     加载 mappings/*.yaml + docs/03 家族指纹 → agent few-shot 上下文
├── tools.py      agent 工具（拉表/字段选项/全表 distinct/读语料/推荐阈值/校验/产出）
├── core.py       agent 编排：拉表 + 枚举 → 起草 → 自查 → 出 draft yaml + review brief
├── eval_wtg.py   WTG 结构回归 eval（见 §验收）
├── cli.py        命令行入口（--dry-run 看 prompt；真跑连飞书 + LLM）
└── README.md
.github/workflows/
└── onboard-table.yml   workflow_dispatch(project_id, feishu_app_token, feishu_table_id) → 跑 core → 开 PR
```

工具（agent 可调，全部只读 / 受控写）：飞书拉表 + **字段选项 / 全表 distinct**（枚举型
列取完整取值）· 读 `mappings/` + 词表 / 家族指纹 · 推荐阈值 · 校验（词表 + D-021 覆盖）·
`emit_draft` 产出。飞书走镜像 `scripts` 的 REST `FeishuClient`。
