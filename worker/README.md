...
| POST | `/annotate-features` | `{project, limit?=50, dry_run?, reannotate?, run_tag?, single?, code_only?, model?, note_ids?, done_by?}` | 内容特征层抽取 pass(docs/28 §5, D-070): 20 道模型题按组问 + 8 个代码特征 + 3 道占位题, 落 `note_feature_answers`。`run_tag`/`single`/`model` 给闸一用(`model` 不许是 `jev:…`/`llm:…` 这种 extractor 标签, D-085); `note_ids`(≤200 个, 必须属于 `project`)只跑这几篇, 响应多一个 `note_ids_count` —— 调用方靠它确认 worker 已认识这个字段(旧版本会吞掉它按 project+limit 跑别的笔记); `done_by`(D-085, 如 `["jev:1.13.0","llm:%"]`)是续跑判据「谁答过就算答过」, 不给就是 `llm:%`, 响应回显 `done_by`(用法同 `note_ids_count`), 要和 `count_unannotated_features.py --done-by` 给同一个值 |
