...
## D-065 续 · §11 十件事拍板（09-19）：策略上的由 Claude 判，业务上的六件问运营

**日期**: 2026-09-19
**触发**: owner —— *"你帮我仔细思考完成所有的判断，如果需要运营结合上实际业务回答的，用一个直白的 word 让运营去填……你需要确定是运营能回答的业务问题，策略上的大的点你自行判断"*。
**状态**: 十件里两件已做（5、10），七件已定（1、2、3、4、6、7、9），一件定了方向、呈现方式待 aw 维护者（8）。逐条见 docs/28 §11.1。运营问卷 `data-analysis/ops-request-2026-09-19.md`（+ .docx）。

### 判的时候查了什么

- **标题（第 2 件）不用问运营。** 17 张表实查：TGV 有独立标题列（130/144 行）；其余 16 张文案里都带标记，两种写法：`【标题】…【正文】…` 11 张，`标题：…\n正文：…` 5 张（NRT_2 / NRT_3 / NUC / HXZ_QD / HXZ_FB，冒号全角半角都有）。docs/28 §3 说「其余表是一整段文案，第一行是不是标题要逐表核实」，实际比那好得多。mapping 键定为 `title_extraction: column | markers | none`，`markers` 一个解析器认两种写法。
- **闸三线（第 6 件）从 0.6 改 0.5。** 依据 D-067 用清洗后口径重算的分档表：最低档 2.23% vs 其余 6.93%，0.32 倍。0.6 是按 9/16 清洗前的 0.45 倍留余量定的，现在基线本身就到 0.32，0.6 等于不设线。
- **运营 9/17 答应的两件，落地了一半。** 「维护情况」在岸深 / 百健士 / SPX 开始填了（64 行），但取值是自由多选（关注 / ✅ / 控评&置顶✅ / 已评待置顶 / 重点关注⭐️ / 带日期的 🍟 / 已投流），投流那组选项没成固定项；「维护时间」只有 SPX 9 行；「维护效果」0 行；途鸽「铺评完成」勾选框没加。这些进了问卷 Q1 / Q2，不是新要求，是追上次的。

### 问运营的六件（只问业务实际，不问策略）

| # | 问什么 | 为什么只有运营知道 |
|---|---|---|
| Q1 | 途鸽「铺评完成」勾选框加了没 | 9/17 答应的，库里没看到 |
| Q2 | 「维护情况」列每个取值的含义；投流会不会成固定选项；「没填」= 没做还是不知道 | 自由多选，含义只有填的人知道 |
| Q3 | 17 个项目品牌词 / 产品词在文案里的实际写法 | `brand_in_title` / `brand_first_position` 的词典；登记的正式名文案里不用 |
| Q4 | 18 条候选写作台指令：哪些客户不允许（写项目）、哪些本来就在做、哪些不该做 | 客户合同约束；「不点名产品」「争议判断」两条对品牌露出有影响 |
| Q5 | 禁词 / 合规清单 | `efficacy_promise` 的例子；只做标记 |
| Q6 | 闸一人工标注：100 篇 × 20 题（约 6 小时）+ 50 篇双人（约 3 小时），出几个人、哪周、飞书还是 Word | 人力 |

### 不问运营的（原来 §11 里看着像要问的）

- 题目增删改：闸一就是改题机制，人和人对不上的题会被逼着改；运营只答 Q4 那张「客户允不允许」的表。
- revise 还是 reject：TV 不拦发布是 R-007 定的，用 `revise`；只有「标记怎么呈现给选稿的人」要和 aw 维护者定。
- 探索比例：15%，batch 级随机。这是实验设计，不是业务判断。

### 接下来

P1 开工不等运营回卷：迁移 `notes_v1_13`（按 §11.4 分工）、`annotate_feature_pass.py`、worker 端点、7 组 CI 守卫、`title_extraction` 解析器。Q3 的词典和 Q4 的约束回来再填进问题库 / mapping；Q6 定了人再排闸一的人工那一半。

### Codex review（09-19，PR #139）· 六条全部属实：拍板结论没同步回 docs/28 正文

| # | 打在哪 | 改法 |
|---|---|---|
| ① P1 | 附录 A 仍带着草案第一版逐字照搬的 `v_l2_labels`（a/b/c/d 四处问题都在）。v1_13 一 apply 就把生产上 v1_15 修好的口径盖回去 | **删掉**，附录 A 只读 `v_l2_labels`；头注、§5.5 表同步 |
| ② P1 | §6.3 仍写 ≤ 0.6 倍、§8 仍写 1,540 篇 | §6.3 改 0.5；样本量按 2.23% / 6.93% 重算：最低档 212、其余 847，约 1,060 篇；附录 D 保留旧行标「作废」 |
| ③ P2 | §7.3 仍写 revise / reject 待拍板，§7.4 仍写 10%–20% | 改成已定：`revise`；15% |
| ④ P2 | 决定「surface 30 个月」被读成对全部 20 题，问题库里两题仍是 essence/60 | **是我写得不清**：决定只针对 18 道结构类题；`judged_by_others` / `negative_outcome_happened` 对应 essence 词表的恐惧 / 羞耻撬动，本来就该 essence/60，问题库不改。§4.3、§11.1、问题库头注改清楚 |
| ⑤ P2 | §3 第 4 件、问题库 `spans.title`、§11 第 2 条仍是「只有 SPX 有标记、其余逐表看」和 `bracket_markers / first_line` | 三处都改成实查结果和 `column / markers / none` |
| ⑥ P2 | §7.2、§10 P0、§11 第 10 条仍把书架挡铺评工单写成待办 | 标已做（D-066 / D-068），§7.2 保留当时的判据作记录 |
...
| 抽取 pass | `scripts/annotate_feature_pass.py` | 库 I/O + LLM（复用 `annotate_essence_pass.call_claude`：中转站、prompt caching、退避）+ 编排。每篇 31 行（20 模型题 + 8 代码 + 3 占位），不问的题也落 NULL + 原因；校验不过的题**只重问那几题**一次；**任何一组** api 挂就整篇不落行、计 systemic；退出码口径同 essence（全军覆没才红）。`--run-tag` / `--single` / `--model` 给闸一 |
| 计数 | `scripts/count_unannotated_features.py` | 直接复用 pass 的 `fetch_done_ids`，算**集合差**（真实 note_id 减去已跑过的），不是两个计数相减 |
...
8. 编排：31 行/篇、8 次调用（6 组 + 2 次重问）、**任一组挂整篇不落行**（反证：换回「全挂才不落」会写进 31 行）、**写序 raw→answers**（反证：对调顺序立刻红）、resume 判据分 code / llm 两套（反证：code-only 用模型标记题就永远 resume 不上）、`--code-only` 不调模型、非法 run_tag 退出 2。

本地把 ci.yml 的 python / yaml / sql 三个 job 全部重放一遍（sql 在空库上从 v1_2 套到 v1_13）。

### Codex review（09-20，PR #141）

13 条全部**先复现再改**，每条都补了守卫或夹具（D-051：断行为不断源码）。

| # | 级别 | 文件 | 它说什么 | 复现 / 判断 | 怎么改的 |
|---|---|---|---|---|---|
| 1 | P1 | `daily-sync.yml` | `features_sync` 用了 `continue-on-error`，但末尾「Fail job on any sync failure」没把它算进去 | 成立：读那段聚合，确实只列了 feishu/comments/essence/curate | 加进聚合；头一批认证 / 配置 / 库错的 `exit 1` 现在会把夜跑判红 |
| 2 | P1 | `annotate_feature_pass.py` | 一组 api 挂、别组成功时仍落行；标记题那行一落，下轮 resume 就永远跳过这篇 | 成立：旧判据是 `systemic and not groups_ok` | 改成「任何一组 systemic → 整篇不落行」，下轮幂等重跑整篇 |
| 3 | P1 | 同上 | `--code-only` 的 resume 仍找 `has_specific_time`（模型题，代码路径从不写），done 集永远空 | 成立：`code_rows()` 只写 8 代码题 + 3 占位题 | 代码路径换标记题 `body_len_bucket`；守卫直接断查询里塞进去的 question_id / extractor |
| 4 | P2 | 同上 | 答案与数值原值是两个独立事务，带标记题的答案先落 → 原值那写失败就永远补不回 | 成立 | 调成 `write_raw_counts` 在前、`write_answers` 在后；守卫断写序 |
| 5 | P2 | `feature_bank.py` | 正文超 1,500 字时 `last_para` 是从截断后的前缀里取的，而它不在 `TRUNCATABLE_SPANS` 里 → 结尾类特征拿中段当结尾 | 成立：手造 1,600 字正文，`last_para` 确实不是真结尾 | `last_para` 改从**未截断**正文算 |
| 6 | P1 | 同上 | `frozen_sha256` 是自指的：把算出来的值写回文件，文件字节就变了，冻结这一步根本算不出来 | 成立：`sha256(文件) == 文件里记的值` 无解 | `bank_digest` 改算**规范化**摘要：剔掉 `frozen_sha256:` 与 `status:` 两行（都是冻结动作自己写的，不是题目内容）。守卫真跑一遍「算 → 写回 → 再校验」，并反证「整文件 hash」那种写法必红 |
| 7 | P2 | `count_unannotated_features.py` | 计数脚本在 GitHub 上按本地 `FEATURE_MODEL` 拼 extractor，而模型变量只在 Railway worker 上 | 成立：两边配不一样就永远对不上 | 默认按 `llm:%` 前缀匹配；要精确对某个模型另给 `--extractor` |
| 8 | P2 | `feature_bank.py` | 表情符计数把变体选择符 U+FE0F 当独立字符，`❤️` 算 2 个；ZWJ 连起来的一家人按部件计 | 成立：实测 `❤️`→2、`👨‍👩‍👧`→3、`👍🏽`→2 | 先剥掉变体选择符，再按「基字 + 肤色 + ZWJ 序列」整体匹配；`[R]` 那种方括号表情另算 |
| 9 | P2 | `librarian/core.py` | `latest_gate2_run` 只在 `validated` 行里挑最新，新一轮全判 `no_signal` 时缓存版本串反而钉在旧轮 | 成立——而这正是最该失效缓存的时刻 | 去掉 status 过滤，按 `decided_at` 取最新一轮；查询失败仍 fail-open |
| 10 | P2 | `feature_bank.py` | choice 题只有一句 `aw_instruction`，却对每个 validated 取值都发一遍（`opening_type=提问` 会收到「用具体事件开头」） | 成立 | 问题库加 `aw_instruction_values` 声明这句话对哪些取值成立，没声明就不自动下发；bool 题只认「是」，且不许有这个字段 |
| 11 | P1 | `notes_v1_13_content_features.sql` | `v_feature_contrast` 把 `bank_sha256` 丢了，分组只按 version/bank_version/extractor → 换题面期间两批答案被悄悄合池 | 成立 | `bank_sha256` 进 vals / JOIN 键 / 输出 / GROUP BY。CI 夹具加了 q2 的 sha/sha2 两档，反证：从分组键里去掉它，`q2:是` 会从 1001+1000 并成 2001 |
| 12 | P1 | `count_unannotated_features.py` | 剩余量是两个计数相减，而答案表对 notes 没外键：删一篇旧的、来一篇新的正好抵消 → remaining=0，两个工作流从此不再调 worker | 成立 | 改成集合差：真实 note_id 减去 `fetch_done_ids` 的结果 |
| 13 | P2 | `feature_bank.py` | `instructions_for_desk` 只按 question_id 找结论行，改过题之后旧版本的结论会套到新题干上 | 成立 | 加两道闸：`question_version` / `bank_version` 都要等于当前问题库的值，缺字段一律不下发 |

顺手自己抓到的一条（不是 Codex 报的）：步骤名里带 `": "` 时，`- name: 甲: 乙` 会被 YAML 解析成 key=`name: 甲`，文件仍是合法 YAML，但这个 step 没有 `name` 这个 key —— GitHub 会**整份 workflow 判非法、一个 job 都不跑**，本地 `yaml.safe_load` 却是绿的。把五个这样的步骤名整个加了双引号，并在 yaml job 加了一条守卫扫所有 workflow 的 step key。
...
### worker 已经自动上线（实测，09-20 05:56–05:59 UTC）

worker service 是「连本 repo」建的（`worker/README.md` 部署段），Railway 这类 service 推到被监听分支就自动重新构建部署。05:38 合并进 main 之后没人动手，端点就在了。

**验法**：手动触发一次 daily-sync（`project=WTG_phase1` + `dry_run`，[run #176](https://github.com/TangVarie/truth-vault/actions/runs/35492918651)），特征那步会 curl 一次 `/annotate-features`。整 run 绿，那一步的两行是：

```
─── features annotate (worker) · project=WTG_phase1 · 每日上限=12 · 每请求≤6 ───
  HTTP 200 · ok=true · Done: {"ok": 6, ..., "title_how": {"bracket": 6}, "truncated": 0}
  HTTP 200 · ok=true · Done: {"ok": 6, ..., "title_how": {"bracket": 6}, "truncated": 0}
```

顺带白捡的三条：

- **新的 `markers` 标题解析器在 12 篇真实 WTG 笔记上 12/12 走 `bracket` 分支**（`【标题】…【正文】…`），`truncated: 0`（没有正文超 1,500 字，与 09-19 实查的「最长 1,013 字」一致）。
- **dry-run 确实什么都没写**：跑完 `note_feature_answers` 仍 0 行，`note_features` 里也没有 `extractor_version LIKE 'fq-v0.1%'` 的行。
- 预算逻辑对：`FEATURE_LIMIT=12`，两批 × 6 跑满即停。

⚠️ **这一条前后写错过一次，教训记下来**：第一版写的是「Railway worker 还没重新部署」——那是**没核实就当成事实**。真相是我拿不到 `WORKER_URL` / `WORKER_API_KEY`（GitHub secrets），所以当时能说的只有「大概率已自动上线、但没核实」。owner 直接问「这东西不是 github 更新之后自动拉取的吗」，才逼出上面这次实测。同 D-051 / 上面 #109 那条的另一面：**「代码进了 main」不等于「运行时跟上了」，但「我没查」也不等于「它没跑」**；两边都只能靠真的去够一次。
...
## D-071 · 角度台账漏账：`/health` 那个 91% 是两种病糊在一起，而且没排除 TV 导入的副本

**日期**: 2026-09-20
**触发**: owner —— *"工作台除了让他们更新 workbuddy 以外还要做什么？"*。查 deskcore 生产 `/health` 时看见 `config.pipeline.ok = false`：近 7 天发牌 214 个角度、只有 20 个销了账，泄漏 91%（健康约 20%，超 50% 该查），4 个项目单独超线。顺着挖下去。

### 挖出来的三件事

**① 194 个没销账的角度是两种完全不同的病**，`/health` 的一个百分比把它们糊在一起：

| 形状 | 角度数 | 会话里发生了什么 |
|---|---|---|
| 抽完没写 | **138** | `draw_angles` 之后同项目窗口内一篇真稿都没入库（追查见下「④」：**不是散了，是稿子在写作台外面写的**）|
| 写了不带坐标 | **56** | 稿子进了指纹库，但提交时没带 `angle_key`，台账销不了账 |

按会话拆（19 个会话）：sportsix 三次共 60 个角度、0 篇稿；hatherine 四个项目同形状；`Hatherine痘痘贴` 09-14 抽 10 写 6 但一个坐标都没带；`hatherine-D2` 09-16 抽 20 写 10 同样不带。**途鸽是唯一全程按协议走的项目**（12 抽 / 8 销 / 12 篇全带坐标）。两个写手各占一半：`85f5f888` 124 个、`afbaf84e` 70 个。

**② 那个 91% 本身就读不得 —— 分母里 97% 不是写作台写的。** 同窗口 1,507 条指纹里 **1,468 条**是 `tv-sync` 把 TV 笔记复制进写作台留下的副本（`autowriter.tv_note_links.match_kind='ingested'`，D-064），它们本来就没有 `angle_key`（`deskcore/core.py` 两处明写「历史稿不是发牌产出的，没有坐标」）。真稿只有 **39 条**，其中 22 条带坐标。不排除它们，未归因率永远是 97%+，这盏灯等于没有；更坏的是**病因会判反**——sportsix 会被诊断成「写了 438 篇都不带坐标」，而它其实一篇没写。

**③ 危害不是少写了几篇。** 没销账的坐标只占坑 1 天（`store.recent_angle_keys` 第二档，这条设计是对的，没被破坏），一天后同一个故事会被再抽一次，而「同题重写」指纹闸抓不到（它抓的是抄句子）。`core.py` 的注释里记着这事真发生过：途鸽 09-10 那批 66 条里 49 条没带坐标，当天四个标题各入库两次。

**④ 追查「抽完没写」到底停在哪（09-20 补）**：第一版写的是「会话停在发牌之后就散了」——**那是猜的，而且错了**。按天把「发牌 / 写作台真稿 / 该项目当天发布的笔记」对起来看：

| sportsix | 09-10 | 09-11 | 09-12 | 09-13 | 09-14 | 09-15 | 09-16 |
|---|---|---|---|---|---|---|---|
| 当天发布 | 7 | 12 | 12 | 10 | 12 | 13 | 4 |
| 其中经过写作台 | 7 | 12 | 11 | **1** | **0** | **0** | **0** |
| 当天发牌 | — | — | — | — | 20 | 40 | — |

**断崖在 09-13，而当天和之后照样每天发 12-13 篇，也照样在写作台发牌。** 所以会话没散——他们继续用写作台**发牌**（那一步有用），但写稿和交付走了别的路，成品只在发布之后由 `tv-sync` 倒灌回来（sportsix 438 条 `ingested` 里 114 条是 9 月发的，雷诺考特 48 条、岸深 133 条、Hatherine 77 条，都不是老存货）。Hatherine 与途鸽是间歇性的同一形状；百健士-藻油 09-13/14 两天 11/11 全走写作台，说明这条路本身是通的。

**断点精确在 `commit_drafts`。** 写作台的价值链是「规则注入 → 发牌 → 写 → 查重 → 入库 → 导出交付」，他们只用了前两环。原因不神秘：**入库不在交付路径上**——交付是把稿子给客户/排期，`commit_drafts` 对写手当下没有回报，回报（查重、跨批避重）都归下一批的自己。典型的「对个人无即时收益、对系统有收益」缺口，靠提醒补不上，得把它挪进交付路径（例如规定交付物一律由 `export_drafts` 导出，那就必须先 commit）。

⚠️ 这张表只看到 09-16 为止：`tv_note_links` 的匹配靠 aw 每日 04:00 UTC 的 `tv-sync`，近 48 小时只新增了 2 条链接，所以 09-17 之后的「经过写作台 = 0」是**匹配还没追上**，不是证据。

### 做了什么

**TV 侧（本仓，PR #143）**：`scripts/check_angle_ledger_leak.py` + 夜跑 advisory 步。两条判据各带下限（发牌 ≥ 10 而真稿 = 0；真稿 ≥ 5 而带坐标 = 0），**先排除 `ingested` 副本再算**。advisory 不拖红——修在 aw 仓，TV 红了也改不好（同 D-063 那盏灯的口径）。CI 守卫钉的 反证：不排除导入副本，sportsix 的病因会从「抽完没写」翻成「写了不带坐标」。

**写作台侧（aw PR #86）**：`build_writing_brief` 在每场对话开头回显 `counts.angle_debt` ——「你自己近 7 天抽了没出稿的角度有几个」，大于 0 时带一句该怎么处置。`/health` 报的是运维视角、没人天天看；写手看得见的地方只有简报。fail-open：查不到只是少句提醒，绝不拖垮简报。协议与 SKILL.md 同步（`protocol_version` → `e7f853cb526e`）。

### 顺手修掉的一处过期协议

`protocol.md` 原文写着「现存库里 300 多条规则的 severity 全是 `soft`，一条 hard 都没有」，据此告诉模型 `hard_rules = 0` 属于正常。实查：全库 465 条规则里 **hard 已有 53 条**（47 confirmed）。这句话会让模型把真的配置问题当成正常，已改。

### Codex review（09-20，PR #143）

三条，全部复现后成立、全改了。两条是这盏灯自己会读错数的路子：

| # | 级别 | 它说什么 | 判断 | 怎么改的 |
|---|---|---|---|---|
| 1 | P2 | `fetch_all_pages` 的 `order_by` 传了 `created_at` / `drawn_at`，不唯一 | **成立，而且现在就会触发**。那个函数按 `order_by` 的值去重（`fresh = 没见过的那些`），它自己的 docstring 明写要求「唯一 + 稳定」。本窗口 7 天就有 1,507 条指纹，早就跨过 1000 行的页边界 | 改成按 `id` 排序并把 `id` 加进 `select`。守卫的假件照抄了真函数的去重语义，**反证**：键换回 `drawn_at`，12 条同刻的发牌塌成 1 条、整个 p2 项目消失 |
| 2 | P2 | `by_project` 用项目名当键，名字不唯一 | **成立**。service_role 读的是全租户的行，生产里就有「百健士-藻油」与「百健士藻油」这种 | 改成按 `project_id` 聚合，名字只作显示。**反证**：两个同名项目各 3 篇不带坐标的稿（都低于门槛），并成一格就是 6 篇、越过门槛冒出假警报 |
| 3 | P2 | 判据 B 会喊一个「压根没发过牌」的项目 | **成立，而且是我刚修过的同一类错**（把导入副本当成没带坐标）。没走 `draw_angles` 的项目，稿子本来就没有坐标、台账里也没有行可销；全局那道 `drawn == 0` 的闸只在**所有**项目都没发牌时才拦得住 | 判据 B 加 `drawn > 0`。**反证**：去掉它，一个 0 发牌 / 50 篇稿的项目会被报成台账漏账 |
...
### 推上去之后立刻红了：第三次「本地绿、线上拒整份」

第一次 push（`ed7f8f7`）的 CI [run #832](https://github.com/TangVarie/truth-vault/actions/runs/35506190735)：**0 个 job、0 秒、run 名字显示成 `.github/workflows/ci.yml` 而不是 `CI`**——GitHub 拒收了整份 workflow。

原因是新加的那条聚合守卫里写了：

```python
sh.replace("<美元><两个花括号> steps.%s.outcome }}" % i, ...)
```

**GitHub 在【加载 workflow 时】就解析 `run:` 里的每一处表达式**——它不认 shell 注释，也不认这是 python 字符串字面量；`steps.%s.outcome` 里的 `%` 不是合法表达式 → 整份被拒。本地 `yaml.safe_load` 对它完全无感（那就是合法 YAML），上一条 step-key 守卫也看不见（key 全对）。

**这是本仓第三次栽在同一类事上**：

| 次 | 形态 | 本地 | 线上 |
|---|---|---|---|
| 1 | 步骤名 `- name —— 甲: 乙`（冒号没引号） | `yaml.safe_load` 绿 | 拒整份 |
| 2 | 同上，五处 | 绿 | 拒整份 |
| 3 | `run:` 里出现一处不合法的表达式 | 绿 | 拒整份 |

新守卫「workflow 里每一处表达式都得是合法的」：**走解析后的结构**遍历所有字符串（步骤名 / `run` / `if` / `env` / `with`），每处表达式检查非空、无非法字符（`%` 是最容易从 python 格式化串漏进来的那个）、上下文根名已知。

写它的时候又踩了一次同一个坑——**把那三个字符写进了自己的步骤名里**，而步骤名同样会被求值。所以守卫里那个模板是 `OPEN = "$" + "{{"` 拼出来的，不是写出来的。

第一版还扫的是**原始文本**，于是把 `onboard-table.yml` / `preflight.yml` 里两条 **YAML 注释**误报成坏表达式——YAML 注释 GitHub 根本看不到。改成走解析后的结构就两边都对了：YAML 注释消失，而 `run:` 块**内部**的 `#` 行仍是字符串的一部分、照样被查。

反证五条：`run:` 里的 `%` / 空表达式 / 根名拼错（`stpes`）/ 步骤名里的坏表达式，四条各红一条；YAML 注释里写它，不报。

⚠️ 连带教训：`ed7f8f7` 的 commit message 里我写了「本地重放 ci.yml: python + yaml 两个 job 共 70 步全绿」。**那句话本身没错，但它不能推出「线上会绿」**——本地重放跑的是各步骤的 `run:` 内容，跑不到「GitHub 收不收这份 workflow」这一层。这个仓已经因为这件事红过三次，而每次本地都是绿的。

### 两个「留给下一次」，owner 让我定——都定成「别留给下一次」

我原先写的是「哪天再逼近，该拆 comments/essence」和「先看 240 够不够，不急着接告警」。
**这两句和 `timeout-minutes` 旁边那句「实测 15-38min」是同一种句子**：都把"回头量一次"
押在某个人某天会想起来。上一句这么写的，裸奔了几个月，直到撞上去才被发现。所以两条都改成
让机器自己喊。

**① 用时灯（`daily-sync.yml`，advisory 不判红）**

开跑时记一个时刻，收尾时报 `用时 / 240min = N%`，**过 70% 就 `::warning`**，并直说下一步
是拆 comments/essence 而不是再把数字往上加。不判红——用时长不是故障，是预告（D-053 不许
天天红）。守卫钉住 `BUDGET_MIN` 必须等于 `timeout-minutes`，改一个忘一个会判红；也钉住这盏
灯不许出现 `exit 1`。

**② 看门狗（`sync-watchdog.yml`，新，23:41 UTC）**

不接任何外部服务。关键在于**它站在 job 外面**——夜跑末尾那道聚合站在 job 里面，有两件事
结构上做不到：

| 它抓不到的 | 为什么 |
|---|---|
| 撞 timeout 被 cancel | job 结论就是 cancelled，聚合步 `exit 1` 盖不过去 → 不发邮件（run #177 本尊） |
| cron 压根没触发 | 没有 job，就没有任何一步会跑。GitHub 对长期没提交活动的仓会自动停 schedule，那种"安静"和"一切正常"长得一模一样 |

看门狗查的是「最近 26h，每个定时 workflow 有没有**一趟成功的 schedule 跑**」，有问题就
`exit 1`——**它自己是个普通失败的 job，GitHub 照常发邮件**。这就把"cancelled 发不出邮件"
这件事绕过去了，代价是零外部依赖。

⚠️ 诚实说清它的边界：**看门狗自己也受 60 天不活跃规则管**。它不是万无一失的兜底，是把
「没人天天看 Actions」的代价从"静默丢数据"降到"某天没收到邮件"。

守卫两条（各带反证）：`WATCHED` 名单必须覆盖仓里**每一个**带 `schedule` 的 workflow（漏一个
= 那趟 cron 停了没人知道，而看门狗照样报绿——正是它要防的那种安静）；以及把看门狗那段
python 原样抠出来喂四种假响应自检：都成功→绿 / 全失败或取消→红 / cron 没触发→红 /
新 workflow 还没跑过→放行并说明。

### 「交付一律从 export_drafts 导」——我提错了问题，撤回

我把这条列成「等 owner 拍板」的待办，owner 反问：*"不是已经说了几次吗？直接从飞书最终版本
里面对不行吗？改了几次了，这次的卡点又是什么？"*

**没有新卡点。这个问题不该被重新提出来。** 查了三件事：

| 查的 | 结果 |
|---|---|
| 404 条 lineage 是哪条路来的 | **430 条全部来自 tv-sync 指纹回写，飞书粘六列贡献 0 条** |
| `v_model_comparison`（lineage 唯一的下游）有什么 | 10 行，但其中 **399/400 个版本的 `ai_engine` 都是 `deskcore`** |
| 谁读 `v_model_comparison` | **没有人**。dashboard 只读 `v_dash_*`；其余引用全是 schema 定义 / 文档 / CI 的存在性检查 |

三条各自都足以否掉我那个建议：

1. **「直接从飞书最终版本里面对」不是一个待定方案，它就是现在跑着的东西**，而且是唯一
   起过作用的。手抄六列那条路 runbook 早就标了作废（三周零匹配），我等于把一件已经办完
   的事又拿出来问了一遍。
2. 就算把覆盖率从 35% 提到 100%，`v_model_comparison` 也答不了它自己那个问题——
   **deskcore 写的每一篇 `ai_engine` 都是同一个值**，「比较不同模型的爆款率」在 deskcore
   时代结构上就不成立。多灌只会让 `deskcore` 那一行的分母变大。
3. 没有消费者。**和 D-072 同一条判据**：没有消费者的表，不值得为它改人的操作习惯。

**决定：不规定。** 交付走什么路继续由写手自己定。

角度销账那一头另算——D-071 已经把 `angle_debt` 摆进简报了，那是不需要改流程的修法。
**先让它真上线**（见下条 Railway），看几周简报提醒本身能把 194 推到多少，再谈要不要加硬规矩。
在有那个数之前提流程规矩，是拿别人的操作成本去赌一个我没量过的效果。

⚠️ 这条的教训不在结论，在**我为什么会把它当成待办**：它是我自己跨轮次拎过来的一句话，
在仓里 grep 不到任何出处（runbook / deskcore 都没有）。**「我上一轮记下来的待办」不等于
「这件事真的悬着」**——交接的清单也要对着仓库核一遍，否则会凭空造出需要 owner 拍板的事。

### 代班 review（Codex 配额用尽，改用 code-review skill）——八条全属实

PR #144 标 ready 的同时，Codex 回了「usage limits」。**这一版加的守卫机器最多，却正好没有
第二双眼睛**，所以换 `code-review` skill 跑了一轮。八条逐条复核，全属实，全修：

| # | 位置 | 问题 | 为什么要紧 |
|---|---|---|---|
| 1 | `sync-watchdog.yml` | 一个 `except Exception` 把 403/502 也吞成「跳过」 | **看门狗会在一个 workflow 都没验过的情况下打勾**——正是它要防的形态 |
| 2 | 同上 | `conclusion == "success"` 把【还在跑】(null) 当失败 | cron 实测漂 5.5-6.3h，features-sync 12:47 + 180min 完全可能在 23:41 还没跑完 → 隔几天误报一封，然后没人再看这些邮件 |
| 3 | `ci.yml` WATCHED 覆盖面守卫 | 判据是全文 substring | **和本 PR 在修的「被注释喂饱」是同一个**：把 WATCHED 里一项注释掉，守卫照样绿 |
| 4 | `ci.yml` 表达式守卫 | `LEGAL` 漏了花括号，而 `ROOTS` 认 `format` | 合法的 `format('{0}', …)` 会被判红 |
| 5 | `daily-sync.yml` | 开跑时刻记在 checkout+pip 之后，灯又排在中间 | 分子系统性偏小；守卫只钉了分母 |
| 6 | `DECISIONS.md` | 条目重复，「两件」下列了三条 | 我自己编辑时留下的 |
| 7 | `README.md` | 「六个 workflow」表 | 漏 `backfill-features`，还把 `gateway-probe` 记成「定时」（它只有 `workflow_dispatch`）。现在九个，已重写并注明**没有守卫钉它** |
| 8 | `ci.yml` | 未使用变量 | —— |

1/2/3 各补了反证；1 和 2 的分支原来**自检根本没走到**。

#### 修的过程里又犯了两次同一个毛病，都记下来

**（一）断言自己被别的消息喂饱了。** 给「API 查不动要判红」写的断言是 `"查不动" in out`，
而 ⑦ 那条兜底消息里也有「查不动」三个字。于是把 403 误判成良性 404 时，⑦ 仍然让整体判红、
`out` 里仍然有那三个字，**断言照样过**。反证连跑三次都是绿的，直到我把那一段单拎出来打印
真实输出才看见。改成钉那条分支独有的一句话。

**本 PR 通篇在修「只查字符串在不在」，结果我在守卫的断言里又犯一次。**判据必须钉在
**只有这条分支才会产生的东西**上，不是「某个词出现了没有」。

**（二）多改一处的脚本在中途断掉，前面的改动一起没写盘。** 一个 python 脚本里串了五处
替换，第四处 `assert old in s` 失败 → 脚本抛错退出 → **`p.write_text()` 根本没执行**，
可前三处我已经按「改完了」记下了。后来反证不咬，才回头发现文件里压根没改。

教训：**多处替换要么各自独立写盘、要么失败时明确报「一处都没写」**；而「脚本打印了 ok」
不等于「我想改的每一处都改了」——得回去 grep 一眼改没改上。

### 第二轮代班 review：七条又全属实，其中四条是**我上一轮的"修复"自己挖的**

`#145`（修上一轮八条的那个 PR）标 ready 时 Codex 又是 usage limits，于是又跑了一轮
`code-review`。七条全属实，**前四条都是上一轮那批修复引入的新洞**：

| # | 我上一轮改的 | 引入了什么 |
|---|---|---|
| 1 | 给 `LEGAL` 放行花括号（为了让 `format('{0}', …)` 过） | **`${…}` 里写 `steps.{0}.outcome` 又合法了**——run #832 那个形态换个占位写法而已，守卫等于自废。而全仓 grep 下来**没有一处**真的用 `format('{0}', …)`：拿真防护换了一个假想的便利 |
| 2 | 加「还在跑不算失败」 | 先判 running 就 `continue`，窗口里同时有 `[还在跑, 失败]` 时**把那趟失败整个吞掉并打勾** |
| 3 | 加 `verified` 兜底 | `verified += 1` 放在拿到结论之前，**只是"还在跑"也算验过**——兜底被它撑起来，正好盖不住它要防的形态 |
| 4 | 用 `status` 白名单判「还在跑」 | 白名单漏了 `pending` → 又报成「没有一趟成功 (最近一趟: None)」，**正是第 2 条要杀的误报** |

另外三条：⑤ 守卫只钉了用时灯的分母，分子那个修法能静默退回去；⑥ 自检里 `error=` 会让
**所有**调用都报错，于是「一个验成了 + 另一个 403」从没测到——而那恰恰是 `verified==0`
兜底**盖不住** 403 分支的场合；⑦ 上一轮修重复条目时把「两件」改成了「三件」，结果标题下
只有两条，**一个计数错换成了另一个**。

全修，并补了四条自检（在跑+失败→照报失败 / `pending`→当没跑完 / 单个 403 不被别人的成功
掩盖 / 在跑的不算验过）和六条反证。

#### 这一轮真正的教训

**「修复」和「新功能」一样会引入缺陷，而修复的复核强度通常低得多。** 上一轮八条我都跑了
反证、跑了全量重放、CI 绿——然后这七条里有四条就藏在那批修复里。反证只证明「我想到的那种
退化会被抓住」，证明不了「我没想到的那种」。

具体到形态，四条里有三条是同一个毛病：**我在分支里加了一条新路径（还在跑 / 放行花括号 /
verified 计数），却没问"这条新路径会不会把别的判据绕过去"**。加路径比改判据危险，因为
它默认是"多一种放行"。

还有一次现场演示：写第 1 条的修复注释时，我又把「美元+两个花括号」原样写进了 `run:` 块
（**这是同一天第三次**），当场被自己那条表达式守卫拦下。守卫是有用的——前提是它自己没被
放宽。

### 第三轮代班 review：**没有正确性问题**——这一轮就是收手的信号

`#146` 标 ready 时 Codex 又是配额用尽，第三次换 `code-review` 跑。这一轮的结论值得单记：

> **No correctness bugs found.** 第二轮指出的四条回归确实修好了；而且 reviewer
> **自己做了变异测试**——把四个修复逐个改回去重跑自检，四次全被抓住。

只剩两条标注/覆盖问题，都修了：

- 自检用例 ⑪ 的注释写着「404 跳过」，实际 `{"daily-sync.yml": []}` 走的是**「从来没跑过」**
  那一支，**404 那支根本没被覆盖**——它哪天开始计入 `verified`，这条发现不了。拆成
  ⑪a（没跑过）+ ⑪b（真 404），并补了反证：给 404 分支加一句 `verified += 1`，⑪b 当场红。
- 步骤名还写着「四种情形」，实际跑 11 个。改成不写个数，并在标题里留一句「加用例时别再
  往标题里写个数」——这和本轮在修的「三件/两件」是同一类计数错，第三次了。
...
|---|---|---|
| 1（审 #144 的内容） | 8 | — |
| 2（审 #145＝第 1 轮的修复） | 7 | **4** |
| 3（审 #146＝第 2 轮的修复） | **0 个正确性问题** + 2 条标注 | 0 |

**第 2 轮那个 4/7 是这件事真正的教训**：修复的复核强度通常远低于新功能，而它一样会引入
缺陷——第 1 轮那八条我全跑了反证、全量重放、CI 绿，照样埋了四个。

**第 3 轮的 0 则是收手的依据**：不是「我觉得够了」，是连续一轮查不出正确性问题、且守卫
经得起别人做变异测试。没有这个信号就一直审下去，那是另一种形式的没有判据。

⚠️ 还有一件贯穿三轮的事：Codex 因为配额用尽，**连续三个 PR 一次都没 review 过**，而这
三个 PR 加起来改了守卫机器的大半。「有个 review bot」和「这次真的被 review 了」是两件事，
跟「代码进了 main」vs「跑着的是那一份」同构——都得去够一次才知道。

### 顺带结掉的两件

- **09-13 sportsix 那天发生了什么**：owner 判「管不了，大家都不记得了」。**结案，不再查。**
  D-071 里那张逐日表留着当记录，但别再拿它去追问当事人。
- **`FEATURE_LIMIT` 仍是 12**：拆出来之后它有 180 min 可用，调大是随时的事（repo variable），
  但闸一还没定人、问题库还可能改题，现在多抽只是多攒一批可能要按新题面重跑的答案
  （docs/28 §10）。等闸一。
...
## D-073 验收 · 看门狗第一次跑就抓到了它被造出来要抓的那件事

**日期**: 2026-09-21

D-073 合并当天，三个定时任务都跑了第一趟。三件都按设计发生，逐条对账：

### ① 看门狗（`sync-watchdog.yml` run #1，01:37 UTC）—— **failure，而且是对的**

```
· 内容特征层增量抽取 (features-sync.yml): OK (2026-09-20T16:30:40Z)
· 夜跑(飞书入库 / 评论 / essence / 策展 / 通道1) (daily-sync.yml):
  最近 26h 跑完了 1 趟, **没有一趟成功** (最近一趟: cancelled)
##[error]1 趟有问题  →  exit 1  →  GitHub 发邮件
```

它指的就是 9-20 那趟 run #177——**同一趟 job 里，末尾那道失败聚合打印的是「✅ All sync
steps succeeded.」**。站在 job 里面的闸说全绿，站在外面的这盏灯说没有一趟成功。**这正是
D-073 要补的那个洞，第一次跑就复现了。**

措辞是「**跑完了** 1 趟」——那是 #146 那条 `done` 修复的产物（先分清「跑完了但没成」和
「还没跑完」，不能先判 running 就 continue）。说明线上跑的是修好的那版，不是 #145。

⚠️ 它会**每晚继续红**，直到 daily-sync 有一趟成功的定时跑。这不是误报，是欠着的账还没还。

### ② `features-sync.yml` run #1 —— success

| | |
|---|---|
| 起跑 | 16:30:40 UTC（排程 12:47，**漂 3.7 小时**） |
| 用时 | **67.1 min** / 180 上限（37%） |
| 产出 | **120 篇**、3,720 行（LLM 2,400 + code 1,320） |
| 无效率 | LLM **1.86%**、code 0.26%（上一批是 2.67%） |

累计 **210 / 4,968 篇 = 4.2%**。按 120 篇/趟，还要 **~40 趟**。比拆之前估的「60 多个夜跑」
快，因为它不再跟主干抢时间、跑满了自己的预算。

**拆对了的证据不是"它成功了"，是这 67 分钟原本要从夜跑那 120 分钟里出。**

### ③ 漂移：本仓 cron 稳定漂几小时，而且**每个 workflow 漂得不一样**

| workflow | 排程 | 实际 | 漂 |
|---|---|---|---|
| daily-sync | 02:17 | 07:05–07:43（近 6 次） | 4.8–5.4h |
| features-sync | 12:47 | 16:30 | 3.7h |
| sync-watchdog | 23:41 | 01:37 | 1.9h |

D-073 里挑 12:47 的理由（「隔半天才是真的不撞，相对关系保证不了」）**被这组数据坐实了**：
两边各漂各的，实际变成 07:20 和 16:30，仍隔着 9 小时。要是当初按「排在夜跑后面半小时」
设计，5.4h 和 3.7h 的差就足够把它们撞到一起。

### 顺带修正一条我自己的判断

18:44 那次自查我看到 features-sync「一次 run 都没有」，**差点报成故障**。是先去量了
daily-sync 近 6 次的真实漂移（4.8–5.4h）才没下这个结论——当时才晚 25 分钟。

**「到点了没跑」在这个仓里不构成证据，要等够一个漂移周期再判。** 这条已经写进了下一次
自查的判据里（到 18:44 仍零 run 才是真问题），也正是那次判据救了这一回。


---

## D-074 · 借阅超时定成 60 秒，并且**把它变成可测的**——这个数已经猜错两次了

**日期**: 2026-09-21
**触发**: owner 选 A（「走A吧，不然这件事没有意义了啊」）——宁可等，也要贴题的卡。

### 先说为什么不能再猜

| 次 | 值 | 结果 |
|---|---|---|
| 原值 | 8s | 冷路径必超时。TV 算完 4-5 张写进缓存，写手一张没拿到，**每次都在白烧 token** |
| 第一次修 | 30s | 实测**仍不够** |
| 这次 | 60s | 见下 |

9-21 那次三步实测给了**仅有的两个样本**：

- 步骤①（新 `draft_topic`，冷路径）：**22 秒成功**，借到 4 张；
- 步骤③（又一个新 digest，还是冷路径）：**超时 0 张**——而 `flywheel_librarian_cache`
  在 03:40:08 有那一行、4 张卡。**又是「TV 干完了、写手挂断」。**

步骤③的调用不早于 03:39:18（draw_angles 的时刻），结果 03:40:08 落地 →
**那次冷路径在 (30, 50] 秒之间**。

60 覆盖这两个样本并留余量。**但它仍然是从 2 个点外推的**，所以这次同时做了下面这件事。

### 真正的修法：让它自己把分布交出来（v1.16）

`flywheel_librarian_cache` 加一列 `select_ms`，**只在冷路径写**（命中缓存那次压根没调
LLM，记 0 会把分布污染成一堆 0——守卫钉死了它不许是 `NOT NULL DEFAULT 0`）。

于是下次定这个数不用再拍脑袋：

```sql
select count(*) n,
       percentile_cont(0.5)  within group (order by select_ms)/1000.0  p50_s,
       percentile_cont(0.95) within group (order by select_ms)/1000.0  p95_s,
       max(select_ms)/1000.0 max_s
from truth_vault.flywheel_librarian_cache where select_ms is not null;
```

**攒够样本（≥20 次冷路径）就回来按 p95 重定，别再用 60 这个外推值。**

之前查不到耗时的原因很实在：deskcore 侧算了 `elapsed_ms` 但只打进 stdout（Railway 日志，
查不了），馆员侧一处耗时都没记。**「没有人记过」是三次拍脑袋的共同前提。**

### 代价，说清楚

`open_project` 是**必经调用**，借阅在它的同步路径上。60 秒意味着 TV 挂住时每次都要等
60 秒；若 MCP 客户端自身超时低于 60 秒，**整份简报（含 P0 硬约束）会一起丢**——这正好
违反 `deskcore/core.py:360` 自己写的那条不变量。

选 A 就是接受这个代价：**卡不贴题等于白借，而现在的状态是大部分时候一张都拿不到。**
已知的缓解是 `LIBRARIAN_TIMEOUT_SEC` 是 env 变量，出事时改 Railway 即时生效、不用发版。

⚠️ 我们**只测到过客户端容忍 ~22 秒**（步骤①那次）。60 秒之上客户端认不认，**没有证据**。
这是这条决定里最大的未知数，不是「应该没问题」。

### 顺带

- 本地终于把 `sql` job 跑起来了（本机有 Postgres，之前一直报「跑不了」是我没试）。
  **全量 122 步**（python 72 / sql 45 / yaml 5）全绿，含新加的 v1.16。
- 改 `put_cache` 签名时打到了 TV-06 守卫的假件（5 参 lambda → `TypeError`，被 try 吞掉
  变成断言失败）。假件改成收 `**kw`——下次真签名再加字段不会再假红一次。
- 写 v1.16 的反证时先跑错了一版：只跑那一步而没跑前面的迁移，三次（含还原）都因
  「schema 不存在」而红。**又是失败原因不是我声称的那个。**改成每次跑完整 sql job 才咬对。
### D-074 续 · codex 在 #149 上抓到一条：我漏登记了 `select_ms`，而那条规矩**只写在注释里**

Codex 配额恢复后审了 #149，一条 P2，**属实**：

`put_cache()` 新写的 `select_ms` 没加进 `scripts/_common.py::_REQUIRED_COLUMNS`，
`_COLUMN_MIGRATION` 也没有 v1.16 的映射。

后果是**静默的**：服务先于迁移上线时，upsert 带上库里没有的列 → PostgREST 拒掉
**整条**写入（42703）→ `put_cache` 外面那层 `except` 吞成一条 warning → **缓存永远写不进去**
→ 每次借阅都重跑一次 22-60 秒的 LLM 选卡。而 `assert_db_schema_ready` 本该在开跑前就拦住
它——前提是这列在清单里。

（本次生产没踩到：我是先 apply 迁移再合代码的。但换个环境、恢复个库、或者下次有人顺序反了，
就会踩。）

**真正值得记的不是"我漏了"，是那条规矩的状态**：`_REQUIRED_COLUMNS` 上方白纸黑字写着
「新增写入列时**必须**同步加到这里 —— 否则这道闸就漏掉了它，事故会原样重演」，而
**执行它的东西一个都没有**。一条只写在注释里的纪律，等于一条迟早会被违反的纪律；我就是
那个违反者。

所以这次补的不只是那一列，是**让它可执行**：新守卫用 `ast` 从 `put_cache` 里解析出所有
真写入的字面量键名，逐个要求在 `_REQUIRED_COLUMNS` 里、且新列能在 `_COLUMN_MIGRATION` 里
指到迁移文件。判据从**代码真的写了什么**推，不是照着清单抄。

反证三条，各咬对：① 把 `select_ms` 从清单删掉（**复现我这次犯的错**）→ 点名报它；
② `put_cache` 新写一列不登记 → 点名报它；③ 登记了但没给迁移映射 → 报说不出该跑哪份迁移。

⚠️ 本地重放时还撞到一次假故障：45 个 sql 步骤齐刷刷 rc=2，看着像我改坏了 schema ——
实际是**容器里的 Postgres 自己停了**（`pg_isready` 说 no response，重启后全绿）。
**「一大片同时红」通常是环境不是代码**，先看连不连得上再看 diff。

## D-075 · 「越来越接近屎山」——量完之后发现不是屎山，是地图没了（2026-09-21）

owner：「这个说起来不复杂的系统，由于里面的细节很多、涉及到的库和环节也不少，现在
变得越来越难以理清，越来越接近屎山。」
...
| | TV | aw |
|---|---|---|
| 产品代码 | `scripts/` 11,058 行(24 个) + `librarian/` 737 | `deskcore/` 9,107 + 根目录 19,082 |
| 守卫机器 | `ci.yml` **7,713** + 其余 9 个 workflow 9,109 | `ci.yml` 2,785 + `tests/` 12,510 |
| 散文 | `docs/` 11,005(31 份) + 本文件 4,742(84 条) | `docs/` 2,934 |

库：`truth_vault` 19 表 + 14 视图，`autowriter` 19 表，`public` 5 表 + 18 视图 = **43 表 32 视图**。
增速：最近两天 **73 个文件、+12,321 行**（其中相当一部分是我加的）。

**TV 的守卫机器（16.8k 行）已经比被守卫的东西（11.8k 行）大 42%。**

### 但结论是：不是屎山

三条可检验的理由：

- **边界干净。** TV/aw 分仓、scripts/librarian 分层、core/store 分层。那天挖出的所有
  断点都是「数据流向没人知道」，**没有一处是「改这里炸那里」**。屎山的定义是耦合失控。
- **注释异常好。** 每条守卫都写了为什么存在、踩过什么坑、反证是什么。这是屎山的反面。
- **它当天抓到三件真事**：`select_ms` 的 schema gate 漏登记、看门狗第一次跑就抓到
  cancelled、TV-06 假件的 `TypeError`。

真正的病是**地图缺失**：「哪条路写哪张表」没有任何单一权威来源，于是每次动手都要
重新考古 —— 那天一个上午去生产库考古了三次（查重比哪张表、坐标在哪销账、缓存为什么
必然 miss）。**这跟屎山是两种病，治法完全不同。屎山的错误治法就是推倒重来。**

### 做了四件

1. **归档 7 份**带日期的交接 / 审计到 `docs/archive/`（只移不删）。
2. **`docs/00-START-HERE.md` §3 换成两张 mermaid 数据流向图**（采集侧 / 写稿侧），
   并撤掉索引里「`26-handover` 是当前状态最新权威」这句 —— 三个月前的交接被当成今天
   的权威，那就是那张错地图本身。
3. **`docs/29-lights-registry.md`**：每盏灯谁看、多久看、红了谁负责。
4. **`scripts/check_system_map.py`** 三条守卫，接进 `ci.yml`（只留一行调用）。

### 两个过程中被证伪的判断，都值得记

**① 「23 份文档六月之后没改过 = 死了」——错的。**

按修改时间列出 23 份准备整批归档，移之前先查了入链：`scripts/_common.py` 正引用着
`02-schema-v1.md` / `03-mapping-protocol.md` / `09-system-integration.md`，
`annotate_essence_pass.py` 引用 `06-essence-annotation.md`，`librarian/README.md` 引用
`14-channel2-pull-librarian.md`。**「半年没动」也可能是「稳定且仍然正确」** —— 规格本来
就不该天天变。照原计划整批移走，会把活代码的引用改断、并归档掉仍然正确的规格，
**比不归档更糟**。

判据换成**「它提到的东西还在不在」**，归档范围收窄到「由构造决定就是历史」的那 7 份
（带日期的交接与审计快照）。`docs/archive/README.md` 里把这条教训写下来了。

**② 第一版 G1 扫全文猜标识符 —— 当场自证会误报。**

把 `sync_feishu_notes_to_truth_vault.py` 里的 `truth_vault.py` 当成表名；把文档里提到的
autowriter 仓的 `core.py` / `app.py` 当成本仓缺失的脚本。**一个会误报的守卫比没有守卫
更糟**：它会被人关掉，然后真问题也没人管了。改成只认 `<!-- map-guard -->` 标记块里的
`tv:` / `script:` / `wf:` 显式前缀。

### 三条守卫各自挡不住什么（写进脚本里，免得用的人高估它）

- **G1** 挡表/脚本/workflow 改名或删除而图没跟上。**挡不住语义漂移** —— 图说「A 喂 B」
  而代码改成「A 喂 C」，名字都还在，它一声不吭。它保证的是「图里没有已经不存在的
  东西」，**不是「图是对的」**。`aw:` 开头的表本仓核不了，只能靠人。
- **G2** 钉 `ci.yml` 内联 heredoc **块数**（当前 79）只许降不许升。
  **钉块数而不是行数**：病灶是「守卫逻辑内联在 YAML 里」不是「文件长」；钉行数会把
  改注释都挡掉，那种闸会被拆掉（D-053）。照规矩做（逻辑写进 `scripts/`，`ci.yml` 只留
  调用）块数不变，这条闸根本不会挡你 —— 这次接它自己的那一步就是这么写的。
  值得钉的理由：`ci.yml` 7,713 行 / 79 块，GitHub **整份解析**，本仓已经发生过一次
  「一个占位符写错 → 整份被拒 → 0 个 job / 0 秒」。**守卫机器自己是最大的那个单点。**
- **G3** 打哨兵行 = 一盏夜跑灯 = 必须在登记册里。**挡不住「登记了但没人真看」** ——
  写「owner 每周看一次」而实际没看，它照样绿。

### 新规矩两条（都做成可执行的了）

- **新守卫逻辑写进 `scripts/`，`ci.yml` 只留调用。** ← G2 执行
- **加灯之前先回答「谁看 · 多久看一次 · 红了谁负责」，答不上来就别加。** ← G3 执行

第二条的由来是 D-053 那条没人用过的推论：**没人看的灯等于没有灯**。证据是
`deskcore/core.py` 自己的注释：「`/health` 早就在报这个比例，但那是运维视角、没人天天
看」——那个 78% 报了很久没人看，9-21 第一次有人拆它，才发现里面混了三种完全不同的
形状、其中一种根本不是问题。**装一盏灯的成本是一次性的，养一盏灯的成本是每天的。**

### 明确没做

- **不重写。** 系统在跑、在产出、当天还抓到三件真事。
- **不拆 `ci.yml` 存量的 79 个内联块。** 改守卫机器本身是本仓最危险的动作 ——
  D-073 那轮 review 的 7 条发现里，**4 条是上一轮的修复自己挖的**。先止血，不动存量。
- **不清理 4 个没接进任何 workflow 的孤儿脚本**（`annotate_comment_threading.py`、
  `preview_injection_candidates.py`、两个一次性迁移）。记在这儿，另开。
- **本文件 84 条 4,742 行需要索引** —— 没人会在动手前读 4,742 行。另开。

验证：本地全量重放 `ci.yml` **118 步全绿**（python 69 / sql 45 / yaml 4）。
8 条反证逐一咬住对应那条守卫，每次还原后复跑确认回绿。

⚠️ 重放时先撞到一次假故障：python job 一大片同时红，实际是系统 python 没装
`supabase`（venv 里是好的）；随后 sql job 一大片 rc=3，实际是上一次跑已经灌过种子数据、
唯一键撞了。**两次都是「一大片同时红 = 环境不是代码」**（D-074 记过这个形态），
全量重放前必须先重建 `tv_ci`。

## D-076 · features-sync 报了一整轮假绿——一句过期的安全假设（2026-09-21 run #2）

夜跑三件首次验收那天，看门狗按预期转绿（run #1 红是对的、run #2 绿也是对的，**两个
方向都验过了**）。但同一晚 `features-sync` run #2 报 `success`，**实际只成了 2 个项目
24 篇，剩下 8 个全灭**。

### 时间线（日志实录）

```
18:19:57  ANSHEN 批1  HTTP 200 ok  4m43s   6 篇
18:24:40  ANSHEN 批2  HTTP 200 ok  4m48s   6 篇
18:29:29  BJS    批1  HTTP 200 ok  4m58s   6 篇
18:34:27  BJS    批2  HTTP 502     5m02s  ← 撞破 Railway 边缘 5 分钟
18:39:30  之后 8 个项目全部 HTTP 409，各 2-3 秒即被拒
```

### 三层，一层套一层

1. **根因是一句过期的注释。** 那一步写着「6 篇 ≈ 36-48 次调用 **< Railway 边缘 5min**」。
   这句话曾经是对的。语料变长之后每批涨到 **4m43s → 4m48s → 4m58s → 5m02s** ——
   它一直贴着那条线走，第四批破了。
2. **502 的后果不止那一批。** 502 只切断边缘连接，**worker 进程还在后台跑、还握着
   ingest 锁**。于是后面每个项目一上来就 409（IngestBusy），2-3 秒被拒。
3. **然后假绿。** `worker_fail_kind` 把 409/502/503/504 全判 `transient`，只打
   `::warning` 不拖红 → job 报 success。那句「瞬时失败，幂等下轮续」在这里**是假话**：
   它不是瞬时的，是每轮都会在第 3-4 批之后必然发生。

**这跟 D-073 是同一种病**：那次是 `timeout-minutes` 旁边一句过期注释让它裸奔了几个月。
**注释里的安全假设会过期，而没有任何东西盯着它。**

### 改了什么

- `FEAT_REQ_MAX` 6 → **3**（6 篇 ≈ 48s/篇 → 3 篇 ≈ 2.4min，对 5 分钟留一倍余量）。
  吞吐**反而涨**：现在是「跑 2 个项目然后全灭」，改完是 10 个项目各跑满。
- 注释从「估算」改成**实测记录**，并把那四个批次的真实耗时写进去。
- 新守卫 **G4**（`check_system_map.py`）：注释里的「【每请求篇数 = N】」必须等于
  `FEAT_REQ_MAX=N`。三条反证各咬对：改代码不改注释 / 改注释不改代码 / 把那句删掉。

### G4 挡不住什么（写进脚本里）

**它不保证那个数是安全的。** 3 篇哪天也变慢到破 5 分钟，这条照样绿。它保证的只是
「代码和注释说的是同一个数」。真正的安全边界只有实测能给 —— 所以注释里要求写的是
**实测记录**，不是估算。

### 没改、但记下来

- **`transient` 的判据没动。** 连续 8 个项目 409 显然不是「瞬时」，但收紧它有 D-053
  的风险（判太严会天天红）。而且根因修掉之后 502 不再发生，级联也就不再发生 ——
  先修根因，观察两轮再说。
- **502 之后不该继续试后面的项目**（锁还握着，注定 409，白跑 20 秒）。这是个独立的
  小优化，等观察完再定。

### 账

```
笔记 6179 · 已抽特征 234 (3.8%) · 待抽 5945
新增常态 ~10 篇/天（9-17 有过 212 的尖峰，新项目接入）
```

按坏掉的 24 篇/轮要 **425 天**；按修好后的 ~120 篇/轮约 **54 天**。

验证：本地全量重放 118 步全绿（python 69 / sql 45 / yaml 4）。G4 三条反证各咬对，
每次还原后复跑回绿。

## D-077 · D-076 没修好：3 篇一批照样破线，而且连坐原样又来了一遍（2026-09-22 run #3）

D-076 把每请求 6 篇降到 3 篇，结尾写了两句「没改、但记下来」：`transient` 判据不动、
502 之后继续试后面的项目也不动，**「先修根因，观察两轮再说」**。第一轮就给了答案。

### 时间线（run #3，#151 合进 main 之后的第一趟）

```
17:19:27  ANSHEN 批1(当晚第一个请求, 3 篇)  → 17:24:32 HTTP 502   >5m05s
17:24:37  BJS / HATHERINE / LNKT / OKMAN / RIO / SPX / TUGE  → 全部 HTTP 409, 各 4 秒
17:25:11  WTG  批1-4  200 · 2m48s · 2m40s · 3m35s · 3m04s
17:37:19  XIWU 批1-4  200 · 3m15s · 3m53s · 2m45s · 4m03s
结果: 3 个项目 27 篇 (WTG 12 + XIWU 12 + ANSHEN 后台落库 3), job 报 success
```

D-076 那句「3 篇 ≈ 2.4min，对 5 分钟留一倍余量」是**错的**：实测 3 篇一批 53–81s/篇，
最慢一批 4m03s 离线只差 57 秒；当晚第一个请求（疑似冷启动）直接破线。D-076 的 G4
自己就写了「它不保证那个数是安全的」—— 应验了。

### 真正的病不在批大小，在连坐

批大小只决定「破线的概率」。一旦破线一次，后面每个项目都撞同一把锁、各被 409 拒掉、
各自被判「瞬时」、整趟报绿。**单看每一个都说得通，合起来就是这一步形同没跑**。
两晚了（9-21 两个项目、9-22 三个项目），同一个形状。

### 改了三处

1. **批大小 3 → 2**（≈ 1m46s–2m42s，冷启动再加一分钟也在线内）。注释换成 run #3 的
   八个批次实测，G4 钉的数跟着改。
2. **409 不再直接跳走 —— 等锁**（`WAIT_409_SEC=60` × 最多 `WAIT_409_MAX=5` 次）。
   409 的意思是「上一个请求还在 worker 里跑」，一两分钟就放锁。立刻换项目去撞同一把锁，
   就是两晚连坐的直接原因。**502 之后继续本项目**（预算照扣，因为 worker 多半在后台
   把那一批跑完了 —— run #3 岸深那 3 篇就是这么落库的），下一个请求撞锁会走等待。
3. **整趟层面再判一次**：发过请求的项目里，**过半一批都没成 → 红**。这是 D-076 没敢动的
   那一条。它不收紧单个项目的 `transient` 判据（那样会有 D-053 天天红的风险），只在
   「十个里八个没成」这种不可能是瞬时的形状上判红。

另外 `timeout-minutes` 180 → 240：修好之后这一步会真的跑满 10 项目 × 12 篇，
按 81s/篇模拟要 167 分钟，离 180 只剩 13 分钟。**之前 180 从没碰到过，只是因为前两晚
都在第 5 分钟就连坐全灭了** —— 又一个被故障掩盖的限制。

### 怎么验的：先让模拟器复现当晚，再看新代码

写了一个假 worker（单锁、每篇 N 秒、首请求加冷启动、边缘 300 秒切断、切断后照样在
后台写完并占锁到跑完），用 run #3 的参数（冷启动 +130s、70s/篇）跑**旧代码**：

```
旧: exit 0 · 3/10 项目 · 27 篇 · HTTP {200: 8, 409: 7, 502: 1} · 34 分钟
真: exit 0 · 3/10 项目 · 27 篇 · HTTP {200: 8, 409: 7, 502: 1} · 32 分钟
```
...
| 场景 | 旧 | 新 |
|---|---|---|
| ① run #3 的条件 | 绿 · 3/10 · 27 篇 | 绿 · 10/10 · 120 篇 |
| ② 冷启动更慢，新代码第一批也破线 | 绿 · 1/10 · 3 篇 | 绿 · 10/10 · 120 篇（等锁 2 次） |
| ③ 最慢实测 81s/篇 | 绿 · 1/10 · 3 篇 | 绿 · 10/10 · 120 篇 · 167 分钟 |
| ④ worker 整个挂了（全 503） | **绿** · 0/10 | **红** |
| ⑤ 锁一直不放（全 409） | **绿** · 0/10 | **红**（等满 54 分钟后） |

三条反证，每条拆掉一处：拆「409 等锁」→ ② 退回连坐（1/10）但整趟判红接住了；
拆「502 后继续」→ ② 岸深丢 10 篇预算；拆「过半判红」→ ④ 退回假绿。

### 挡不住什么

- **挡不住慢性欠产**：每个项目都只成 1 批，整趟照样绿。那是吞吐问题，要看
  `note_feature_answers` 的日增量（run #3 之前 7254 → 之后 8091）。
- **模拟器是按 run #3 校准的**，不是 worker 的真实模型。它证明的是「在当晚那种故障形状下
  新代码会怎么走」，不证明所有故障形状。
- **根本解不在这里**：runner 开 2.5 小时只为了一个个 curl、等 worker，是结构问题。
  让 worker 自己在后台跑完整轮、runner 只负责发起和收结果，才彻底没有 5 分钟边缘这回事。
  这次没做 —— 范围大，先让现在这条路不说假话。

### 账（更新 D-076）

```
已抽特征 8091 行 · run #3 之前 7254
按 run #3 实际的 27 篇/轮 → 待抽 ~5900 篇要 ~220 天
按修好后的 ~120 篇/轮 → ~49 天
```

### 续（2026-09-23 夜，run #4 实测）：修好了

features-sync run #4（17:29–19:45 UTC，main 已含 D-077 / D-083）：10 个 daily 项目全部发过请求，
**60 批 59 批 200、1 批 502（LNKT，后台照常完成，DB 里 12 篇齐）、0 个项目一批都没成**；120 篇落库
（`note_feature_answers` 2,400 行 llm），每批（2 篇）用时中位约 2 分 8 秒、最长约 3 分 40 秒，整趟 2 小时 16 分。
没有出现 409 等锁——backfill（17:05–17:25）恰好在它起跑前结束。对照 9-21 / 9-22 两晚的 3 个项目 27 篇假绿，
这一趟就是 D-077 要的样子。

## D-078 · audit_log 是唯一精确的指标与 tier 历史 —— 保留策略必须「先抽后删」（2026-09-22）

owner 问「每次数值变化，数据库能精准记录吗」。查下来答案是**能，但不在以为的那张表里**：

- `metric_snapshots`（本来就是记指标用的）按 `(note_id, window_label, source)` upsert ——
  **一个时间窗只留一个数，同窗内每晚覆盖**。7 个窗（2h/24h/72h/7d/14d/30d/final），
  每篇一生最多 7 个点；2h 窗全库 0 行，24h 窗 158 行。窗内的变化会丢。
- `audit_log` 对 notes 的每次 UPDATE 记 `changed_cols = {列: [旧, 新]}`，**只记真变了的**
  （旧 = 新的行数为 0），每晚一个粒度。5/27 至今：指标变化 **10,100** 行、tier 变化
  **8,264** 行。比如 SPX 一条 9/16🍟 的帖：9/16 阅读 778 · 互动 73 → 9/19 曝光 12,081 ·
  阅读 3,796 · 互动 216 —— 投流前后看得一清二楚。

而 `data-analysis/unified-feishu-table-assessment-2026-09-22.md` 的 B6 建议
「audit_log 保留 90 天……现在就该做」，`schemas/notes_v1_2.sql:665` 自己写着 30 天：

| 按什么删 | 删掉的不可再生历史行（指标 / tier / tier_source 变化） |
|---|---|
| 90 天 | **4,442** |
| 30 天 | **6,219** |

同一份评估自己就在用 audit_log 当 tier 历史的唯一来源（「历史真降档 330 个 / 368 次」）。
**照 B6 做，等于把评估的证据源删掉。**

### 规矩

1. **先抽后删。** 任何 audit_log 保留/清理作业，必须先把 `changed_cols` 含
   `impressions / reads / interactions / comments_count / tier / tier_source` 的行
   抽进一张窄表永久保存（只要这几列的 `[旧, 新]` + `row_id` + `occurred_at`，体积很小），
   再删。
2. **噪音是可以放心删的那 95%**：只动了 `updated_at / last_seen_at` 的行。根因是
   `set_updated_at()` 无条件 `NOW()` 让空 diff 短路对 notes 失效（评估风险 9）。
   修掉根因比定期删更好 —— 删只是治标。
3. 这条之前**没有任何保留作业存在**，所以目前没有东西在删。规矩是给将来写那个作业的人的。

### 挡不住什么

- audit_log 的粒度是**每晚一次**：同一天里运营改两次，只留同步那一刻的值。
- 抽出来的窄表也只是「TV 看到的变化」，不是「飞书里发生过的全部变化」。

## D-079 · 闸一人工标注：三份互相看不见的表、每人 50 篇、给模型看什么就给人看什么（2026-09-23）

运营 2026-09-22 回填：三个人、9/28–30。owner：**做互相看不见的表格，尽量简单，大家都很忙。**
方案与名单见 `data-analysis/gate1-human-annotation-plan-2026-09-28.md`、
`gate1-human-sample-2026-09-28.csv`；生成器 `scripts/build_gate1_human_sheets.py`。

### 定了什么

1. **每人 50 篇，不是 100 篇。** 100 篇分 4 组各 25 篇（每组 5 个项目各 5 篇），
   A = 组 1+2、B = 组 2+3、C = 组 4+1 → 50 篇双标（A–B 25、A–C 25）+ 50 篇单标。
   总 150 篇次与 docs/28 §6.1 原计划「100 + 50 再标一遍」相同，但每人只要约 3 小时。
   A 同时在两对里：A–B 对得上而 A–C 对不上 → C 离群。
2. **人看的 = 模型看的。** 只给 `feature_bank.build_spans` 切出的标题 / 正文；
   不给项目名、产品名、爆没爆。模型不问的格子（没标题、正文太短）预先灰掉。
   否则人和模型的分歧里混着"信息不对等"，分不清是题坏了还是模型错了。
3. **表里零公式。** 沙箱里 LibreOffice 连三格的表都算不动，无法重算校验（xlsx 技能的硬要求）；
   进度条不是必需的，白格子本身就是待办。
4. **名单进仓、原文不进仓。** `.gitignore` 默认忽略 `*.csv` 防 PII，它自己写了
   "要版本化的样本放 data-analysis/ 并加逐文件例外"——照做。名单只有 note_id / y / tier /
   组 / 标注员 / 切法 / md5。
5. **原文落盘必须 md5 校验。** 100 篇是从 MCP 结果手抄进文件的，抄错一个字人看的就不是模型
   看的那一篇。逐篇 md5 与库里 `md5(raw_content)` 对上（0 条不符）才允许生成。

### 记一笔：子代理拒绝了抽样任务

第一版想用 workflow 让 5 个子代理各拉 20 篇落盘。**五个全拒了**：workflow 把 owner 最近
一条无关的提问（关于追踪列）当成"用户请求"转给它们，而抽样指令在它们眼里像"批量导出他人
笔记原文到文件、且不让内容出现在返回值里"的可疑模式。它们的判断从各自视角是对的——
是我把任务塞进 workflow 时上下文断了。最后自己拉的，靠 md5 兜底。
教训：把带敏感数据的机械任务交给子代理时，任务描述里要带上 owner 的原话授权和目的。

### 挡不住什么

- 表格分开挡不住三个人坐一起标。
- 「拿不准就选最接近的，不要空着」压低了"不知道"的表达，某些题 κ 会偏高。
- 只覆盖闸一 5 个项目；正例稀少的题在 25 篇重叠里可能算不出 κ（要写"不可算"）。
- NUC / NRT_2 / NRT_3 的模型答案还没跑，落库脚本 A15 还没写——9/28 前不补上，标完了也对不了。

## D-080 · PostgREST 的 `in_()` 走 URL，边缘按整个请求头 ≈26 KB 打 400 —— 列表来自行数的必须分批（2026-09-23）

daily-sync #179（2026-09-22）只红一步：`prepublish_sync`，
`sync_autowriter_decisions_to_prepublish.py` 查「这些 item 已归档了哪些评价」时把窗口内
**全部** autowriter item id（623 个 UUID）塞进一个 `.in_()`，边缘回裸的 `400 Bad Request`
（不是 PostgREST 的 JSON 错，supabase-py 包成 `JSON could not be generated`）。
前一晚 #178 同一段代码 613 条过了；#177 是 D-073 已修的超时，不是这个。

### 实测（anon key 走同一条 builder 链的探针，prod 边缘）

| in_() 里 UUID 数 | URL 字节 | 额外请求头 | 结果 |
|---|---|---|---|
| 613（#178 的量） | 24,178 | — | 42501（通过边缘，被权限拦，正常） |
| 623（#179 的量） | 24,568 | — | 42501 |
| 623 | 24,568 | +2,000 B | **400 Bad Request** |
| 630 | 24,841 | — | 42501 |
| 635 | 25,036 | — | **400** |
| 500 | 19,771 | +16,000 B | **400** |

结论：限制在**整个请求头（URL + headers）≈ 26 KB**，头也算。623 条在沙箱里离线只差
~300 字节；夜跑的请求头（service_role JWT、httpx 0.28 的默认头）比探针的多几百字节就够
撞——这一句是推断，runner 上的头没量过；量到的是上面那张表。一个 UUID 在 URL 里占 39 字节（逗号编码成 `%2C`），
窗口是 365 天、只增不减，每天 +10 条：不分批就是一颗按日走的定时炸弹。

第一次「加头不影响」的探针是**空的**——头根本没上线（`pad_sent=0`）。事件钩子里量到
真实字节才敢下结论（D-051：每条守卫都要反证，探针也是）。

### 定了什么

1. `_fetch_existing_evaluations`：主流程与归档复核共用，按 `_IN_LIST_CHUNK = 200` 分批
   （≈7.8 KB，离线三倍余量，也在 nginx / Cloudflare 常见的 8 / 16 KB 口径之内）。
2. CI「评价来源不得伪造成人工 (TV-01)」⑨：假 builder 在列表超 24 KB 时抛同款 400；700 条
   item 跑主流程和复核，都必须一条不少地拿回。反证：`_IN_LIST_CHUNK=10_000` 红、只把
   复核那处改回单次查询也红。
3. `_common.fetch_all_pages` 的 docstring 记下这个数——它有 24 个调用点，列表来自行数的
   都得分批。已在分批的：`sync_feishu_notes_to_truth_vault.py`（100）、
   `check_angle_ledger_leak.py`（200）。

### 挡不住什么

- 26 KB 是**今天**量到的边缘行为，不是文档承诺；哪天收紧到 8 KB，200 条一批仍在线内，
  但 `evaluator_type` 之类的短列表不在这条规矩里。
- 修的只是这一处。别的脚本要是也把「窗口内全部 id」塞进 `in_()`，会在各自的行数走到
  600 出头时同样红——docstring 只能提醒，挡不住。
- 这条修好之后夜跑才会绿；在 #152 合并前，每晚 `prepublish_sync` 照旧红（其余步骤不受影响）。

## D-081 · 闸一"人工那一半"由 Jev 顶替；模型答案按指定 note_id 跑，不灌整个项目（2026-09-23）

owner 拍板：**「Jev 落库（直接替代人，jev 比人更准），100 篇跑起来」。**

背景：D-079 的三张标注表 2026-09-23 回来了，是模型 jev-1.13.0 填的（表里自述），不是人填的。
对名单核对全对（三张各 50 篇、灰格对、闭集内、0 空格）；重叠 50 篇上同一模型自洽 κ 0.91–1.00。

### 定了什么

1. **Jev 的三张表落 `note_feature_answers`**：`extractor = jev:1.13.0-A / -B / -C`（一张表一个，
   否则重叠篇互相覆盖）、`run_tag = gate1-20260928`；Jev 自报"把握不够"的格子把概率写进 `prob`。
   落库走 `scripts/ingest_gate1_answers.py --sql-out`（沙箱没有 service key，SQL 经 MCP 执行；
   与 `--write` 的 upsert 逐行等价，本地 tv_ci 里先跑过一遍再上生产）。
2. **闸一"测得准"的对照组改为 Jev**：docs/28 §6.1 的"人 vs 人 κ ≥ 0.60"这一半，按 owner 的判断
   用 Jev 顶替。**如实记**：Jev 与 TV 模型的一致率是**模型 vs 模型**，不是人 vs 模型；两个模型
   同时错在同一处（题面歧义、切段差异）时这个数看不出来。9/28–30 三位同事若仍标，同一脚本
   `--extractor human:<姓名>` 照收，届时三方对照。
3. **TV 自己的模型只跑这 100 篇**：worker `/annotate-features` 加 `note_ids`（≤200，必须属于
   `project`，响应带 `note_ids_count`）；`backfill-features.yml` 加 `note_ids` 输入——按 id 前缀
   自动分项目、每 `batch` 篇一请求、先用 `dry_run` 探针确认 worker 认识 `note_ids`（旧版本会把
   这个键吞掉、按 project+limit 去烧别的笔记）。不灌整个项目：5 个项目共 2,592 篇，
   其中 NUC / NRT_2 / NRT_3 是 on_demand，跑完 100 篇要 ~2 小时，跑 2,592 篇要两天多。

### 反证

- worker：note_ids 缺 / 空 / 非法字符 / >200 / 非字符串 → 400 且不起子进程；无 key → 401。
- workflow：id 不带 `_rec` / 项目无 mapping / 含 `;` → 校验步红；旧版 worker（响应无
  `note_ids_count`）→ 探针即停，0 个真请求；新版 → 100 篇 = 5 项目 × 10 批 × 2 篇，探针带 `dry_run`。
- SQL：A+B+C 在本地 tv_ci 内 999/999/1000 行、prob 123/124/113、重跑 A 行数不变（覆盖不是再插）。

### 挡不住什么

- Jev 顶替人之后，闸一再没有"人怎么看"这个锚；题面本身模糊的地方两个模型会一起错。
- 按 note_id 跑要 worker 先部署新版本（Railway 从 main 自动部署）；探针只能挡"旧版本"，挡不住
  "部署到一半"——那种情况探针会 502，重试即可。

## D-082 · 闸一 95 个分歧格由 owner 裁决：TV 对 66 / Jev 对 24 / 都不对 5；五题题面升 version 2（2026-09-23）

D-081 跑完 Jev vs TV 一致率（`data-analysis/gate1-jev-vs-tv-2026-09-23.md`）之后，真不过的 5 题共
95 个分歧格（Jev 三张表合并后与 TV 模型答案不同的格子）做成一题一页的审阅表（含原文、两边答案、
TV 证据、题面），owner 当天逐格裁决并写了理由。裁决落库：`note_feature_answers`，
`extractor = 'human:owner'`，`run_tag = 'gate1-20260928'`，`question_version = 1`，`evidence` = owner 的理由。
**这是闸一到今天为止唯一的人工锚**（D-081 说过 Jev 顶替人之后没有"人怎么看"这个锚，现在这 95 格有了）。

### 裁决结果

| 题 | 分歧格 | TV 对 | Jev 对 | 都不对 | 对 owner 的一致率（分歧格按裁决、其余格假定两边都对） |
|---|---|---|---|---|---|
| 前后转折 turning_point | 29 | 26 | 3 | 0 | TV 0.97 · Jev 0.72 |
| 产品角色 product_role | 20 | 16 | 3 | 1 | TV 0.96 · Jev 0.82 |
| 交代身份 narrator_identity | 16 | 4 | 12 | 0 | TV 0.88 · Jev 0.96 |
| 拿别人对照 comparison_group | 11 | 10 | 1 | 0 | TV 0.99 · Jev 0.90 |
| 第一句类型 opening_type | 19 | 10 | 5 | 4 | TV 0.91 · Jev 0.86 |
| 合计 | 95 | 66 | 24 | 5 | |

⚠️ 右列的"一致率"只裁了分歧格，两边一致的格子没有人看过；两个模型一起错的地方这个数看不见。
它回答的是"分歧处谁对"，不是"整体准不准"。

**D-081 里"jev 比人更准"的前提在这五题上不成立**：四题是 TV 对得多，Jev 的阈值松
（一件事 + 求助就算转折；求推荐就算产品出场；泛论就算对照）。只有交代身份一题是 Jev 的口径
对（老公 / 孩子 / 男朋友这类称呼推出家庭角色、工位 / 主管推出上班族、跑过几次半马是资历——
owner 判"读者知道这是什么人就算"），TV 原来只认年龄 / 职业 / 城市这类硬信息。
...
1. **前后转折**：「之后」必须是**已经发生的**状态变化。求助 / 想改 / 纠结 / 渐变（第一个月还好、
   ...
   每次都这样的规律 / 别人的前后，都不算。往坏的方向转、靠熬过去的，只要前后写明就算。
2. **产品角色**：求推荐、品类用法提问、只在话题标签里、日常食物、打比方、拿不准的备选 = 未出现；
   特指某款不写名 = 只暗示，**没写名字优先于角色**；反面例子一句带过 / 递过来没说用没用 /
   故事起因的补品 = 顺带一提；通篇写味道用量价格 = 主角（开头一句"怎么买的"不改变）；
   品类作为解决办法被提出、医生开的还在质疑性价比的 = 解决方案；品类叫法（NRT）算写了名字。
3. **交代身份**：弱交代都算——称呼推出家庭角色、场景推出职场处境、运动资历、病情 + 处境；
   ask 里加了"处境"。「否」只剩"全文没有任何指向我是谁的称呼、场景、资历"。
4. **拿别人对照**：必须"别人怎样 + 我怎样"两半都写出来。自己比自己、别人比别人、泛论感叹、
   引述别人的评价或意见、只说听说来源、比东西不比人、别人经历只是引出产品，都不算。
5. **第一句类型**：先看主体，开头的情绪词 / 语气词是帽子不定类型；处境 / 习惯 / 慢性状态 =
   身份自述；对别人的判断 / 规律性观察 / 段子概括 = 观点断言；反问梗 = 感叹情绪（不是提问）；
   引某人说的话 = 具体事件，还没发生的事不算；数字是重心 = 数据事实；几个词不成句 = 其他。

### 升 version 的后果（照 D-065 / docs/28 §4.3 的纪律）

- 旧的 v1 答案（全库 `llm:claude-opus-4-6` primary、Jev 三张表、owner 裁决）**保留不覆盖**；
  新抽的答案落 `question_version = 2`，两批在 `v_feature_contrast` 里按 version / bank_sha256 分组，
  不会混。`instructions_for_desk` 按 question_version + bank_version 对版本，这五题的 v1 结论不再下发。
- features-sync 的"跑过"判据是标记题（`has_specific_time`）那一行，**不会**因为五题升版而把全库
...
- `feature_bank.validate_bank` 空错误；CI 特征层守卫 1–7 + 编排自检本地全过（题面里没有表现类词，
  "状态爆好"这种原文用语改成"特别好"，守卫 2 才不抓）。
- 把 owner 理由里的例子放进 yes_examples / no_examples 后，`_render_question` 的输出照旧是
  "算「是」… 例：… / 算「否」… 例：…"，提示词长度每组增加 < 600 字，六组都不超 4 题。

### 挡不住什么

- 95 格是 owner 一个人裁的，没有第二个人；"人 vs 人 κ"仍然没有。
...
## D-083 · 特征抽取「整组 missing」是回复解析失败，不是漏题：容错解析 + 单题兜底（2026-09-23）

owner：「按照你的判断继续」（D-082 续 3 说的"下一件事先修 G6 漏题"）。

### 查到什么

闸一 100 篇两次跑（primary / gate1-v2）校验没过 49 / 75 格，`missing` 30 / 59 格。按篇看，**missing 全是整组
一起没的**：G6 三题一起 6 篇、G4 四题一起、G2 四题一起、G3 三题一起、G1 两题一起——没有一篇是组里漏一题。
同一篇在两次跑里同一组都丢（recuYWaet0pGni / recv0whF4cHJ83 / recv4TC4JgofIx / recuZ8QQXiae2a /
recvmpA0N0x5gu），是内容决定的，不是抖动。14 篇里 10 篇正文含英文双引号 `"`（其余 86 篇只有 21 篇含），
G6 最重——被人评价 / 前后转折的证据恰恰是引别人说的话。代码路径：`ask_group` 用
`annotate_essence_pass.parse_claude_json`（严格 `json.loads`）→ 解析失败 → `validate_answers(parsed=None)`
→ 整组记 `missing`；重问时只说"不是合法 JSON"，模型照样把 `"` / 换行原样抄进 evidence，再断一次。
没有拿到原始回复（worker 不回传），所以"引号 / 换行弄断 JSON"是从整组丢 + 引号分布 + 代码路径推的，
不是看见的——这次改动同时把回复开头记进日志，下次就能看见。

### 定了什么

1. `feature_bank.parse_answers`：去围栏和闲话取最外层 `{…}` → `json.loads` → `strict=False`（允许字符串里
   原样换行）→ 正则逐条捞 `id / answer / evidence` 三元组（evidence 最短匹配到 `"` 后紧跟 `,` / `}`，内嵌
   的 `"` 不截错）。捞出来的仍过守卫 3 的原文子串校验，捞错只会 NULL 不会错答。
2. `ask_group`：解析失败记 `json_parse_failed`（库里和 `missing` 分得开），回复开头 200 字进日志；重问说明
   点名格式问题（`"` 写成 `\"`、字符串里不换行）；**重问后仍「没答」的题按单题再问一次**（回复短、更不容易断），
   单问的结果如实替换（答出来 / 仍 missing / 仍 json_parse_failed）；单问 api 挂 → 那题 `api_error`、整组按 systemic 处理（整篇不落行、下轮重跑；组里任一题停在 api_error 就算，不再是「全组都 api 才算」）；`stats.singles` 计数。正常一篇 0 次单问。
3. SYSTEM_TEMPLATE 加一条规则（英文双引号转义、不换行、只输出一个 JSON 对象）。题库文件没动，bank_sha 不变。
4. 守卫 8 `scripts/check_feature_parse.py`（ci.yml 只留一行调用，文件还剩 ~1 KB 才到 505,000 的棘轮）：
   四种坏回复都能捞、散文 None、捞出的假证据仍被守卫 3 拦；编排上正常 6 次调用 0 单问，坏组 6+1+3，
   漏一题 6+1+1，单问挂不 systemic；反证：严格 `json.loads` 对换行那份必失败、关掉兜底三题全 NULL。

### 反证 / 验证

- 本地：守卫 1–8 + 编排自检全过（原有 31 行 / 8 次调用断言不变——好回复不触发兜底）。
- 生产验证要等 Railway 部署：把这 14 篇按 `note_ids` 落新 `run_tag`（gate1-v2fix）再跑一遍，看 `missing` /
  `json_parse_failed` 剩多少、`singles` 几次——**这一步才是证据**，没跑之前上面的机制只是推断。

### 挡不住什么

- 模型按题号而不是 id 作答、或整段拒答——三元组捞不到，仍是 missing，靠单问兜底；单问也拒就是 NULL。
- 正则捞到的 evidence 恰好也是原文里别处的子串——守卫 3 只查子串不查位置。
- 单问多花钱：只在失败时，一组最多 4 次；按 100 篇 14 组失败算，多 ~40 次调用（600 次分组调用的 ~7%）。

### 续（同日）：自审四条

推送前三视角自审（找 → 逐条反驳），四条成立、一并改了：① 正则路只还原 `\"`，evidence 里合法的 `\n`
会变成字面反斜杠 n → 子串校验必败、好答案被判 evidence_not_found 且进不了单问兜底 → 捞出的字符串体按
JSON 字符串解码；② 证据结尾锚在逗号上，原文 `",` 一截就成前缀，前缀仍是子串、30 字上限只对截断后生效 →
锚在 `"` 后紧跟 `}` 上（evidence 是最后一个键），捞到完整证据交给守卫 3；③ JSON 解析出来但形状不对
（裸三元组、键名打错、answers 是对象、闲话里的 `}` 把 rfind 带偏）直接返回 None、正则层永远到不了 →
从第一个 `{` 解析第一个完整值 + 形状扶正；④ 单问 api 挂只 append 到 errs、既不记日志也不进失败队列，
整篇照常落行、标记题一落这篇永远不再来 → 记 `api_error` + 日志，组里任一题 api_error 即 systemic；落了行
但过程有非致命错误的也记一笔进失败队列（"partial:"）。docs/28 守卫表补第 8 行；"约 2%" 改成 ~7%。

### 续 2（同日 17:05–17:25 UTC）：生产验证——机制坐实

把两次跑里丢过整组的 14 篇按 #156 的代码重跑（backfill-features run 35893206179，`run_tag = gate1-v2fix`，
题库 sha `ba0f570c…`，探针核过 worker 已是新版本）：
...
| missing | **56**（13 篇） | **0** |
| json_parse_failed | — | **0** |
| evidence_not_found / too_long | 1 / 0 | 2 / 1 |
| 分组重问 | ? | 7 组 |
| 单题兜底 | — | **0 次** |

280 格一格不少（14 × 20）。56 格 missing 全部消失，靠的是容错解析 + 点名格式问题的重问说明；单题兜底一次都没
用到（它是最后一道，留着）。所以 D-083 那句"引号 / 换行弄断 JSON"从推断变成了实证：同一批原文、同一模型，
换解析器就全回来了。中途一次 502 是 Railway 边缘超时，重试即过（那批 2 篇第一次已落库，第二次 Found 0）。

挡不住什么（仍然）：模型整段拒答；evidence 恰好是别处子串。下一次全量跑（features-sync 夜跑）里
`json_parse_failed` 应该接近 0，明早回查。

### 续 3（2026-09-23 夜）：第一趟全量夜跑

features-sync run #4 是 #156 合并后第一趟夜跑：10 个项目 120 篇 2,400 格，**missing 0、json_parse_failed 0**，
单题兜底 0 次，分组重问 30 组左右，校验没过 5 格（evidence_not_found 3 / evidence_too_long 2）。
D-082 续 3 里 49 → 75 那个数，在新代码下回到个位数。

## D-084 · owner 逐条拍板：铺评减法重判、ssll 撤回、标签里的品牌、艾时达、TGV 改名、禁词、人工标注取消、Jev 不重跑（2026-09-23）

owner：「需要我这边做的一个个引导我完成了」→ 八件挂着的事一条条问，一条条答。

| # | 问什么 | 拍了什么 | 落地 |
|---|---|---|---|
| B1 | ops-answers §0.4 那 12 行靠铺评过线的「爆贴」 | **12 行一起挡**，口径「评论数 ≥ 50 但扣掉铺评后不够线 = 铺评跨线」 | 实查：这 12 行自 9/22 判据点 ⑥ 上线起已是「铺评工单」，三处都已挡。真正没落地的是它的**反面**：扣掉仍过线的真赢家（BJS 205 / 73、SPX 148）也被同一条路误挡。本次在 sync 加减法重判：`comments_count − seeded ≥ 50` → route 改记「铺评未跨线」（下游只认「铺评工单」，一律放行；seeded 照记）；评论数缺失不减；tier 不动。守卫 ⑬ 九条 + 反证（爆线抬到天上 → 退回铺评工单） |
| B2 | 已推书架 / 策展 / ssll 的 14 行 | **先 dry-run 名单；书架侧撤，ssll 侧先问对面** | 名单 `data-analysis/seeded-overline-list-2026-09-23.md`。如实记：14 行 `synced_to_ssll_at` 全空，D-068 的自愈回收已在夜跑里把它们撤了（最晚 #180，9/23 09:02 UTC），「先问对面」来不及了；书架是实时视图，route 一变即出架，无需另撤 |
| B4 | 品牌词只在话题标签里算不算正文露出（1183 行） | **不算**，与 product_role v2 同口径 | `code_features` 匹配前本来就剥掉标签（ops-answers Q3 已查）。现状即结论，不改代码，记口径 |
| Q14 | OKMAN「艾时达」52 行 | **是自家的**，加进 brand_aliases | `mappings/OKMAN_phase1.yaml`；代码特征下次夜跑按新词典算，旧行不回填 |
| A12 | TGV 产品登记「维他命」0 命中 | **改成「TGV不哭糖」，直接写生产** | 生产 `projects.product` 已改（returning 核过）；mapping `product` 同步改，「维他命」留作 brand_aliases |
| B3 | 7 个禁词落哪 | **compliance 硬拦、全局、「最」只拦绝对化搭配** | **落不了**：7 个词只在运营回填件里（仓内只知「最」「空窗」），且仓内没有任何代码读 `compliance.custom_red_flags`（三个候选落点一个没在用，ops-answers Q5 已查；autowriter 仓代码搜索 0 命中）。要 owner 把 7 个词给 TV，并确认写作台那边谁执行硬拦 |
| — | 9/28–30 三位同事人工标注 | **取消** | 计划文档标取消；脚本保留（Jev 的表就是走它们收的） |
| — | Jev 按 v2 题面重跑 | **不重跑** | 闸一的模型对照到此：owner 95 格裁决 + TV 自身抖动 + Jev v1 历史对照 |

### B1 减法重判的反证 / 挡不住什么

- 守卫 ⑬：49−20 剔、73−20 放、70−20 边界含等号放、69 剔、无评论数剔、工单列 100−50 放 / 99 剔、真赢家 + 起量后干预两路并存、
  通道 1 共用判据对「铺评未跨线」放行、对「铺评工单」仍挡；反证把爆线抬到 10⁹ → 73 退回铺评工单。
- 挡不住：seeded 只认 20 / 50 两档（多档取 max），运营实际铺了别的数量看不出来；「关注情况」列里的「关注」仍不参与（运营没给语义，⑪f 钉着）；
  大爆 扣完落到 50–99 仍记 大爆（减法不重判档位）。
- 生效时间：下次夜跑（daily-sync 02:17 UTC）重算 flags；3 行真赢家回到书架 / L2 / 通道 1 push。

### 续（B3，2026-09-24）：owner 给了 7 个禁词 —— 最、第一、100%、根治、治愈、空窗、绝对

落地按 owner 拍的：**全局、硬拦、「最」只拦绝对化搭配**。

- 唯一来源 `prompts/forbidden_words_v1.yaml`（fw-v1，`scope: global`）；读法只有 `scripts/compliance_words.py`
  （`find_hits` 给写作台 / 脚本用，`sql_regex` 给量库用，两者同语义）；CI 守卫 9 `check_compliance_words.py` 钉住。
- 口径：`match` 原样子串 + `except_followed_by` 例外字。「最」紧跟 近/后/终/初/开/先/早/晚/起/里/外/上/下/前/底/边/中/多/少
  放行，其余（最好 / 最有效 / 最安全 / 最便宜）拦；词在句尾也拦（「这个最」宁可多报）。「100%」连全角 １００％、
  一百分之百、百分之百、百分百一起拦。根治 / 治愈 / 空窗 / 绝对 任何位置都拦。
- **「第一」的序数例外（第一次 / 第一天 / 第一步…放行，第一品牌 / 第一名 / 第一选择拦）是 TV 照「最」的道理类推的，
  不是 owner 原话** —— owner 说了「最」只拦绝对化，没说「第一」；要 owner 确认，不认就把那行的 `except_followed_by` 删掉。
- 守卫 9：7 个词顺序照 owner 给的；「最近很火」不拦、「最好的选择」拦在下标 2；「第一次」不拦、「第一品牌」拦；
  全角 / 中文写法的 100% 拦；每个词的 SQL 正则与 `find_hits` 在同一批样句上判断一致。三条反证：例外表清空 → 「最近」
  被拦；aliases 去掉 → 「百分百」漏网；表里少一个词 → 那个词漏网。
- 用 `sql_regex` 在生产 `truth_vault.notes` 原文（标题 + 正文）上量了一遍命中面（6,197 篇）：

  | 词 | 命中篇数 | 其中 爆 |
  |---|---|---|
  | 最（绝对化口径） | 1,251（20.2%） | 83 |
  | 第一（排位口径） | 171 | 13 |
  | 绝对 | 92 | 6 |
  | 治愈 | 23 | — |
  | 100%（含全角 / 中文） | 10 | — |
  ...
  这是历史原文，不是 TV 的成稿，量的是「硬拦会拦掉多少已经发出去的写法」；五分之一的笔记带绝对化的「最」，
  说明这条拦下去写作台要改的句子不少，爆贴里也有。空窗那 1 篇是途鸽「就业空窗期」，与医美无关，照全局拦。
- **今天还是没有任何一步在成稿前跑这个检查**：TV 仓没有成稿环节；autowriter 仓代码搜索里没有读
  `compliance.custom_red_flags` 的地方（ops-answers Q5）。硬拦要写作台那边接 —— vendor 这个 yaml（记校验和，
  D-041 词表纪律）或直接调 `compliance_words.find_hits`，成稿命中即退回。这一步谁做、什么时候做，等 owner 指派。
- 挡不住什么：「最」+ 例外字开头的绝对化（「最上乘」被「上」放行）；拆开写（「最 好」）；同义改写（「顶级」「No.1」）；
  英文 / 拼音变体。都是子串匹配的边界，不做分词不猜语义，靠人审兜。

### 续（B3 撤回，2026-09-24）：owner 决定整件事不做

owner 看了写作台接法后问「为什么禁词表还有操作步骤？有什么意义」，TV 给了三个选项（只在写稿提示词加一句 / 提示词加代码检查 / 整件事不要），
owner：「整件事不要了。」

- 撤掉：`prompts/forbidden_words_v1.yaml`、`scripts/compliance_words.py`、CI 守卫 9、docs/04 Step 6 那段；交接文档 docs/30 未合并（PR #160 关闭）。
- 留下：上面两段续作为记录，生产命中面那张表也留着（最 1,251 篇等），以后重提时不用再量。
