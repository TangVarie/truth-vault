...
**代价说清楚**：这 27 行会让 `feishu_sync` 每晚继续红。天天红的 CI 等于没有 CI ——
OKMAN 从 8/21 起静默丢行一个月，正是因为那段时间没人相信这一步的颜色。所以这条不能拖着，
要么 owner 定 ①，要么运营在飞书把顺序理回来 / 把「父记录」填上。

**Rejected**

- ❌ **代码里改成「往上找最近一条小红书」**：owner 9/2 明确否掉（「上一条还是抖音就不能判断」），
  且在读法 ② 下会静默给 27 条配错文案。
- ❌ **把 SPX 这 27 条的 `errors` 豁免掉让夜跑回绿**：那就是 D-051 拒绝过的「把红压回去」。
  它们是有链接有指标的真笔记，不是空占位。
- ❌ **改 `ignore_duplicates=False` 让隔离行整条覆盖**：会把运营已经填的 review 状态冲掉。

**守卫**: CI 新增 D-053（OKMAN「评论关键词」进 raw_extra 且与 TUGE 同口径 / 原因串必须带排序后的列名）。
反证过两处：撤回 mapping 声明 → 红；把原因串写回 `"undeclared_fields"` → 红。37 个纯 Python 守卫块全绿。
