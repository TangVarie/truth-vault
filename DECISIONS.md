...
已认领：SPX_phase1 那 27 行（`reason=inherit_prev_row_is_same_kind`，`reviewed_by='owner (via D-053)'`），
全库仅此 27 行非 pending。运营把顺序理回来之后，那些行不再进隔离，名单自然失效。
...
所以：**列名是 `字段 N` 这类飞书默认占位名 → 不进 mapping，走 D 段认领转黄，等运营改名。**
这条闸自己会销账，不用有人记得清理：
