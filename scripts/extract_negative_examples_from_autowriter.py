...
def query_source_a(sb, aw_project_id: str | None = None) -> Set[str]:
    """Return item_ids whose AI version was replaced by a manual rewrite."""
    # PostgREST embedded resource filtering needs care; we use rpc() or
    # multiple queries.  Simplest: pull all manual versions, find their items.
    q = (
        sb.schema("autowriter")
        .table("versions")
        .select("item_id, items!inner(batches!inner(project_id))")
        .eq("ai_engine", "manual")
    )
    if aw_project_id:
    ...
    # Verify that EACH item also has a non-manual prior version
    # (an item could in theory only have manual versions if all writes
    # came from手动精修 — rare but possible; filter those out)
    candidate_item_ids = {r["item_id"] for r in rows if r.get("item_id")}
    if not candidate_item_ids:
        return set()

    confirmed: Set[str] = set()
    # Batch query to avoid N+1
    for batch in _batched(candidate_item_ids, 50):
        res = (
            sb.schema("autowriter")
            .table("versions")
            .select("item_id, ai_engine")
            .in_("item_id", list(batch))
            .neq("ai_engine", "manual")
            .execute()
        )
        for r in res.data or []:
            confirmed.add(r["item_id"])

    return confirmed & candidate_item_ids
...
def query_source_c(sb, aw_project_id: str | None = None) -> Set[str]:
    ...
    pending_rows = (q_pending.execute()).data or []
    ...
    for batch in _batched(batch_ids, 50):
        ...
        approved_batches = {r["batch_id"] for r in (res.data or [])}
        for b in batch:
...
def fetch_already_reviewed(sb, aw_project_id: str | None = None) -> Set[str]:
    ...
    # PostgREST can't do "OR" cleanly across two columns + inner join in one
    # call easily.  Do two passes.
    seen: Set[str] = set()
    for col in ("example_label", "example_label_proposal"):
        q2 = q.not_.is_(col, None)
