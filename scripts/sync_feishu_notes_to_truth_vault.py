...
def main() -> int:
    ...
    if not app_token or not table_id:
        # 未配 feishu sync_config = 该项目【还没 onboard】(mappings 里是占位 null)。
        # 这【不是错误】→ 优雅跳过(exit 0)。否则 daily-sync 跑全量时, 任何一个尚未
        # onboard 的占位项目都会拖垮整个 cron(2026-06-02 实测: NUC/NRT 占位 → cron 全红)。
        # 要接入该项目: 在 mappings/<project>.yaml 的 sync_config 填 feishu_app_token/table_id。
        logger.warning(
            "project %s 未配 feishu_app_token/table_id (sync_config 占位 null) "
