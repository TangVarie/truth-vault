...
数据来源:
    ...
    馆员流量     = truth_vault.flywheel_librarian_cache(last_hit_at 在窗口内; 新 brief 的
                   last_hit_at = created_at, 重复命中只刷 last_hit_at) —— 按 consumer 分列
    ⚠️ 缓存表由 prune_librarian_cache.py 按 30 天 TTL 清理, 本脚本窗口(默认 24h)远在其内。

用法:
    python check_librarian_traffic.py                 # 默认看过去 24 小时(+7 天参考)
...
def report(stats: dict, window_hours: int) -> int:
    ...
    print("=" * 84)
    if by_consumer:
        print("  按 consumer: " + ", ".join(f"{k}={n}" for k, n in sorted(by_consumer.items())))
    print(f"  7 天参考: 写作台 batch={b7} · 馆员流量={hits7}")

    if b == 0:
        print("  · 窗口内写作台没有生成, 通道 2 无从判定(不是健康, 是没活) —— ok")
        return 0
    if hits == 0:
        msg = (f"写作台过去 {window_hours}h 生成了 {b} 个 batch, 馆员一次都没被调 —— "
        ...
        return 1
    print(f"  ✓ 写作台在生成({b} batch), 馆员也有流量({hits}) —— 通道 2 通着")
    return 0
...
def gather(sb, since_iso: str, since7_iso: str) -> dict:
    ...
    return {
        ...
        "lib_hits": len(rows),
        "lib_new_briefs": sum(1 for r in rows if (r.get("created_at") or "") >= since_iso),
        "lib_by_consumer": by_consumer,
        "aw_batches_7d": _count("autowriter", "batches", "created_at", since7_iso),
        "lib_hits_7d": _count("truth_vault", "flywheel_librarian_cache", "last_hit_at", since7_iso),
    }
