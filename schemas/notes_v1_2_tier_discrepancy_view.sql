...
-- ════════════════════════════════════════════════════════════════════
-- ⚠️ 2026-09-17 (D-062): 本文件的定义已被 notes_v1_12_comment_tier.sql 第 5 节【重定义】。
--    判爆只看评论数之后, 这里按 projects.tier_thresholds × 互动量算出来的"矛盾"是过期尺子
--    (它对着生产吐过 175 条)。保留本文件只为部署顺序(notes_v1_2.sql 里的 1c)不变;
--    按 scripts/README Step 0 的顺序跑到 v1.12 会自动换成评论数口径。别再单独改这里。
-- ════════════════════════════════════════════════════════════════════
