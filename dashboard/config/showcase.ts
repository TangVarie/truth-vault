 ...
 * 下面两个 map 仅作【缺 seq 时的兜底】(如看板库 v_dash_projects 还没 seq/category,例如指向旧库):
 * 库里有 seq → 一律走实时 seq+品类(created_at 顺序 / 品类改了也跟着变);只有缺 seq 才回落到这里,
 * 所以这些写死值【不会】盖掉活的 seq/category。新表(库里有 seq)全自动,无需在此登记。
 */
...
/** 对外战线全称:战线 ε · 美妆。数据驱动优先 —— 有 seq 就用实时 seq+品类;只有缺 seq 才回落兜底 map。 */
export function frontLabel(p: FrontMeta): string {
  if (p.seq && p.seq >= 1) return `战线 ${greekOf(p.seq)} · ${CAT_SHORT[p.category ?? ""] ?? p.category ?? "—"}`;
  return PROJECT_LABEL[p.project_id] ?? p.project_id;
}
/** 对外战线短号:ε。数据驱动优先;只有缺 seq 才回落兜底 map。 */
export function frontShort(p: FrontMeta): string {
  if (p.seq && p.seq >= 1) return greekOf(p.seq);
