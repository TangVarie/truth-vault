...
| [docs/28-content-feature-layer.md](docs/28-content-feature-layer.md) | **内容特征层**（D-065 拍板；P1 抽取 pass 已实现 D-070，闸一待运营定人）：原子问题进事实层，过三道闸才进 L2 / 经验卡 / 写作台 |
...
- **L2 预测层从没启动** —— 数据量早就过了 1k 门槛，9/16 做过一次实测（[l2-feasibility.md](data-analysis/l2-feasibility.md)）：
  ...
  **别急着搭服务** —— 实测里「负面情绪撬动 vs 正面共鸣」项目内差 3–6 倍，这条规律本身不用模型就能交付。
  9/19 起有了下一步的设计：[docs/28](docs/28-content-feature-layer.md)（D-065，9/19 拍板，9/20 P1 抽取 pass 落地 D-070）—— 让 LLM 抽发布前可得的结构化特征进事实层，过三道闸才进 L2。
