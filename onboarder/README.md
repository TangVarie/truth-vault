...
**确定性取数 + 单次 Anthropic 调用**(librarian 同款,走中转站)。
⚠️ **2026-08-28 起默认走流式**(`ONBOARDER_STREAM=0` 可退回):非流式对宽表结构上跑不完 ——
网关(CF)的 **120s 读超时**在没收到字节时触发, 而列多的表输出长过这个窗口。实测决定成败的是
**列数**不是行数:LNKT 52 列 ❌ · XIWU 31 列 ❌ · SPX 20 列/294 行 ✅ · BJS 15 列 ✅。
流式让 token 持续外吐, 计时器就不触发。早期那条「非流式已验证能透传」当时没错(它验的是
能不能透传), 只是没覆盖「输出长到 120s 都出不完」这种情况。
