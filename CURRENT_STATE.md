...
下面是经过 Session #10 全系统架构 review 后**明确决定延后但不忘记**的事项。每一项都有触发条件 —— 一旦该现象出现，就该把这项捡起来做。**没有触发就别动**，这是小团队的取舍纪律。

| 延后事项 | 现在不做的理由 | 触发条件（出现就该做） |
|---|---|---|
| **pgvector embedding rerank** 注入候选 | 现有 weighted score（recency+tier+tier_source+account）+ diversity 软约束已能满足 Sprint 0-1 选择质量 | TV 累计 baokuan 数 ≥ 5000，且 v_autowriter_injection_candidates 现有 score 在多个项目同时跑偏 |
| **微调小模型**（embedding adapter / LoRA / 任何端到端学习） | 小团队工作量过大，且没有评估基线就训练等于乱猜 | 招了懂 ML 的人；或现有 ranking 在多个项目同时出现下游 tier 命中率显著下滑 |
| **完整 LLM-judge backtest framework** | 太重；目前 `preview_injection_candidates.py` 的 side-by-side 对比已能让人眼判断"新策略是否更合理" | 决定做小模型时同步上 |
...
| **跨项目 essence 知识迁移**（NRT 的爆款经验喂给 NUC 的 autowriter） | 跨项目 surface-level（品牌/产品名）不能混；essence-level 迁移需要先验证 essence 标注本身可靠 | 至少跑稳 6 个月单项目映射，且新品类启动时缺历史 baokuan 严重影响生成质量 |
...
| **评论作为 prompt 素材的隐私脱敏**（用户名/手机号等） | 内部使用、不外发；评论原文已经在公开平台可见 | 真有合规 / 法务审查；或评论数据要面向客户/外部分享 |
...
| **autowriter Memory Manager UI 负例审核 tab** | 当前 negative example_label_proposal 候选写入了，但 autowriter 没前端审核界面 | 运营反馈 negative_examples 注入对生成质量有帮助、值得审 |
| **Sync 失败 Slack/飞书 webhook 告警** | GitHub Actions 自带的 owner 邮件告警已经够（小团队全员都能收到） | 团队规模扩到 5+ 工程师或失败邮件被忽略 |
...
| **mypy / ruff / sqlfluff 静态检查** | 现有内联 CI 测试覆盖了关键路径；类型错误一般在 py_compile 阶段已暴露 | 团队规模扩到 3+ 工程师，或第一次因为类型错误线上崩 |
| **pytest 框架替代 ci.yml 内联测试** | 当前内联测试可读、可维护、CI 上跑得快；切换成本不小 | 测试数量翻倍（当前 ~10 个），或本地复现 CI 故障变得困难 |
| **完整 RLS 策略 + 多用户隔离** | TV 只有后台 sync 在用 service_role 写，没有用户层访问 | 第一次有 Streamlit / 网页 UI 暴露 TV 数据给非工程师 |

**判断这清单要不要扩**: 任何新发现的"做着做着会出事"风险 → 加进表里 + 写触发条件 + 不要立刻去做。任何"现在好像可以优化但不紧急"的想法 → 写触发条件存这里。**避免被未来可能用得上的事拖住，是这清单的核心目的**。
