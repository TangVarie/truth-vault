...
> 🛠 **2026-09-20 (D-070)**: 内容特征层 P1 落地 —— 迁移 `notes_v1_13`(三表 + `v_feature_contrast`; **生产 09-20 已 apply**, Supabase 记录 `20260920054230`, 见 D-070 续), `scripts/annotate_feature_pass.py` + `feature_bank.py`, worker `/annotate-features`, daily-sync 增量步(每晚每项目 ≤ 12 篇) + `backfill-features.yml`, 17 张 mapping 加 `title_extraction`, 附录 E 七组 CI 守卫。下一步闸一(300 篇, 等运营 Q6 定人)。
> 📝 **2026-09-19**: 内容特征层**设计草案**落仓（9/19 拍板 D-065 续, 9/20 P1 实现 D-070; 下面这段是当时的状态）: [docs/28](docs/28-content-feature-layer.md) + 问题库 `prompts/feature_questions_v0_1.yaml` + D-065(草案)。20 道闭集原子题进事实层, 过三道闸(测得准 / 有区分度 / 对新笔记成立)才进 L2 / 经验卡 / 写作台。附录 A 的迁移只在文档里, 定稿后才落 `schemas/`。写草案时顺手查出三件和特征层无关、现在就能修的事(书架没挡铺评工单 / `v_l2_labels` 口径四处漏洞 / 分档表没用清洗后标签重算), 各自单独开 PR。
> ✅ **2026-09-20 · 生产已 apply 到 v1_13**(编号 13 落仓晚于 14 / 15, 不改; 见 D-070 续): `note_feature_answers` / `feature_validation` / `content_scores` 三张空表 + `v_feature_contrast` 就位, `v_l2_labels` 未动(正例 304 / 负例 4,652)。Railway worker 已自动上线新端点（09-20 dry-run 实测 `/annotate-features` HTTP 200，run #176），今晚 cron 开始每晚增量。
> ✅ **2026-09-19 · 生产已 apply 到 v1_15**: 书架挡铺评工单 (v1_14, D-066): 书架 353 → **312** 张, 41 张铺评工单爆贴下架 (TUGE 40 / RIO 1); L2 正负例口径落成视图 `v_l2_labels` (v1_15, D-067): 正例 304 / 负例 4,652, 以后 l2 的 SQL 只读它。顺手发现 v1_10 六月起就在部署清单里但生产从没 apply 过, 今天一并补上 (D-066 续)。
