...
> 📝 **2026-09-19**: 内容特征层**设计草案**落仓, 待拍板、未实现、没动库: [docs/28](docs/28-content-feature-layer.md) + 问题库 `prompts/feature_questions_v0_1.yaml` + D-065(草案)。20 道闭集原子题进事实层, 过三道闸(测得准 / 有区分度 / 对新笔记成立)才进 L2 / 经验卡 / 写作台。附录 A 的迁移只在文档里, 定稿后才落 `schemas/`。写草案时顺手查出三件和特征层无关、现在就能修的事(书架没挡铺评工单 / `v_l2_labels` 口径四处漏洞 / 分档表没用清洗后标签重算), 各自单独开 PR。
