...
> 🛠 **2026-09-20 (D-070)**: 内容特征层 P1 落地 —— 迁移 `notes_v1_13`(三表 + `v_feature_contrast`; 合并后 apply 生产), `scripts/annotate_feature_pass.py` + `feature_bank.py`, worker `/annotate-features`, daily-sync 增量步(每晚每项目 ≤ 12 篇) + `backfill-features.yml`, 17 张 mapping 加 `title_extraction`, 附录 E 七组 CI 守卫。下一步闸一(300 篇, 等运营 Q6 定人)。
