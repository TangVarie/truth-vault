...
**当前阶段**: 阶段 0 → 基础设施就绪; **daily-sync 全挂根因已查实 = 代码 bug `ClientOptions(schema=None)`, 已修, 待重跑验证 (见 docs/12)**; 飞轮质量阻塞仍是 R-022
**当前会话编号**: #13
...
## 🟡 daily-sync 全挂根因已查实并修复 (2026-05-27, Session #13)

**症状**: 手动跑「Daily TV sync」5 个 sync 步骤全挂 (UI 绿勾是 `continue-on-error`
假象, 末尾红色只是 outcome 汇总器)。

**真根因 (代码 bug, 非配置)**: `scripts/_common.py: get_supabase_client()` 用
`create_client(url, key, ClientOptions(schema=None))`。在 supabase-py 2.30.0 /
postgrest 2.30.0 下, `schema=None` 变成 None 的 Accept-Profile 头, 每次
`.execute()` 在发请求前就抛 `AttributeError: 'NoneType'...encode`。所以 5 步全挂、
两库 REST 日志零流量、truth_vault 至今 0 行 (代码从未真正跑通)、换 key 无效。
**与 secret/项目/飞书/数据库无关 —— key/url 一直是对的。** (先前一度误判成 "secret
配错项目", 是错的; 本地用 2.30.0 复现确认。)

**已修**: 去掉 `ClientOptions(schema=None)`, 改用默认 schema (调用点本就显式
`.schema()` 覆盖)。commit 在分支 / PR #20。

**待验证**: 拉最新分支重跑 → 全绿 + `select count(*) from truth_vault.notes` > 0
(现在 0)。若重跑出现 `Host not in allowlist` (403), 则项目 Data API 开了 IP 白名单,
要加 GitHub Actions IP 段或关限制 (见 docs/12 §3)。
